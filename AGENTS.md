...
- **Deterministic output** — two distinct properties:
  ...
  - *No prior-output dependence* — generation must not read the prior contents of the output
    ...
    idempotent re-export is silent). `--config` mode adds two reads of output AFTER the run, bounded
    the same way — no generated byte depends on either, delete them both and every emitted file is
    identical: the convergence warning (each consumed sidecar's bytes before/after the run, exit 0)
    and the committed-state verdict (each `deps` edge's consumer `borrowed_collections.rs` against
    ...
    the run. Nothing reads prior *tool* output to decide what code to generate, so "run twice = run
