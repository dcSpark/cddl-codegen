...
- **Heavy tiers contend across sessions — coordinate before launching one while another session's
  ...
  os error 28, all green solo). Get the user's explicit go-ahead for a heavy tier when another
  session is active. Related, proven the same day: pointing `GATE_CACHE_DIR` at ANOTHER checkout's
  `.gate-cache` both writes into that checkout's state (do not do this to a checkout you were told
  not to touch) and buys nothing when the checkouts' generated bytes diverge — a run against a
  3-commits-behind master cache scored 0 hits in 41 cells (keys hash the generated tree; different
  emitted bytes = different key, by design). Default to the session's own checkout's cache.
- **Evidence preservation: every multi-minute run leaves its FULL output in a file under
  ...
  which is reserved for investigation/proposal documents. `draft/logs/` is PER-CHECKOUT and a
  WORKTREE's copy dies with the worktree: removing a worktree deletes every log its runs wrote
  (proven 2026-07-19 — an ENOSPC-failure tier log and a 23-cell sweep's logs were destroyed by the
  worktree's own cleanup, leaving a commit message as the only record of the sweep). Before
  removing a worktree whose runs produced evidence worth keeping (any failure sighting, any run a
  commit message cites), copy those logs into the MAIN checkout's `draft/logs/` first.
