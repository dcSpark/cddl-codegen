...
mod tests {
    ...
    fn struct_map() {
        ...
        assert_eq!(
            bar_canonical.encodings.as_ref().unwrap().len_encoding,
            LenEncoding::Canonical
        );
        assert_eq!(
            bar_canonical.encodings.as_ref().unwrap().orig_deser_order,
            vec![2, 3, 0, 4]
        );
            ...
            for (str_3, str_4) in str_3_encodings.iter().zip(str_4_encodings.iter()) {
                    ...
                    [(false, false), (false, true), (true, false), (true, true)]
                {
                    ...
                    let keys = [
                        ...
                        .collect::<Vec<u8>>(),
                        if has_5 {
                            [cbor_int(5, def_enc), cbor_str_sz("text", str_4.clone())]
                                .into_iter()
                                .flatten()
                                .copied()
                                .collect::<Vec<u8>>()
                        ...
                        [cbor_int(1, def_enc), vec![NULL]]
                            ...
                            .collect::<Vec<u8>>(),
                        if has_derp {
                            [cbor_str_sz("derp", str_4.clone()), cbor_int(2, def_enc)]
                                .into_iter()
                                .flatten()
                                .copied()
                                .collect::<Vec<u8>>()
    ...
    fn table_arr_members() {
        ...
        assert_eq!(
            other_order.arr2[0].encodings.as_ref().unwrap().len_encoding,
            LenEncoding::Indefinite
        );
        assert_eq!(
            other_order_encodings.table_encoding,
            LenEncoding::Indefinite
        );
    }
    ...
    fn string64() {
        ...
        let _ = String64::from_cbor_bytes(&cbor_str_sz(
            &(0..64).map(|_| "?").collect::<String>(),
            StringLenSz::Len(Sz::Two),
        ))
        .unwrap();
        assert!(String64::from_cbor_bytes(&cbor_str_sz(
            &(0..65).map(|_| "?").collect::<String>(),
            StringLenSz::Len(Sz::Two)
        ))
        .is_err());
    }
    ...
    fn string1632() {
        ...
        let _ = String1632::from_cbor_bytes(
            &vec![
                cbor_tag_sz(7, Sz::One),
                cbor_str_sz(
                    &(0..16).map(|_| "?").collect::<String>(),
                    StringLenSz::Len(Sz::One),
                ),
            ]
            .into_iter()
        ...
        .unwrap();
        assert!(String1632::from_cbor_bytes(
            &vec![
                cbor_tag_sz(7, Sz::Inline),
                cbor_str_sz(
                    &(0..15).map(|_| "?").collect::<String>(),
                    StringLenSz::Len(Sz::Inline)
                ),
            ]
            .into_iter()
    ...
    fn defaults() {
        ...
        for str_enc in &str_3_encodings {
            for def_enc in &def_encodings {
                for ((key_1_present, key_1_default), (key_2_present, key_2_default)) in
                    bools.iter().zip(bools.iter())
                {
                    ...
                    let irregular_bytes = vec![
                        vec![MAP_INDEF],
                        if *key_1_present {
                            vec![cbor_int(1, *def_enc), cbor_int(value_1 as i128, Sz::Two)]
                                .into_iter()
                                .flatten()
                                .clone()
                                .collect::<Vec<u8>>()
                        ...
                        if *key_2_present {
                            vec![cbor_int(2, *def_enc), cbor_str_sz(value_2, str_enc.clone())]
                                .into_iter()
                                .flatten()
                                .clone()
                                .collect::<Vec<u8>>()
