...
1. **Grow the emitted round-trip harness toward matrix-driven execution.** The `--emit-tests`
   ...
   - **c6 — matrix-driven execution (the F3 frontier): profiles remaining.** Both halves of the
     ...
     unminted shapes are labeled "no minted round-trip surface" rather than credited. The
     preserve/json emitted-test surface now RUNS at corpus breadth via the manual
     `feature_corpus_roundtrips_nondefault_profiles` gate (`--emit-tests` + `cargo test` for both
     profiles, rust + wasm). Remaining: `verify.ts`'s matrix probes still exercise the default
     flags only — extending the matrix "supported" verdict to a per-profile axis (the
     pending-decision entry below) is the longer-term frontier.
