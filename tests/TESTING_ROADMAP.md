...
> **Where we're at / what's next.** **c1 is done** (corpus ✅ 34→52 features, 4→9 control-ops; 3 generator
> bugs surfaced — see `cddl-matrix/ROADMAP.md` findings ledger). The open work, roughly in order: **(a)** make
> the panic-blessing call (Pending decisions) — it unblocks **c2+c3**; **(b)** **wire the gates into CI**
> (§2) — the corpus snapshot/compile gates + the matrix `*.ts` checks all pass but run only manually; **(c)**
> the 3 surfaced generator bugs are quick fixes (exclusive-range is one character) if/when the generator is in
> scope. The remaining ➕ 7 are NOT gaps (intentional excludes + detector-floor under-reports — see c1 Status).
...
- **c2 — unsupported-construct scorecard. _Conditional (gated on a maintainer call, below); ~M._** Project
  the *panic-class* `➖` gaps into a `catch_unwind` snapshot (ok / error / PANIC), mirroring
  `src/robustness_tests.rs::input_robustness_catalog`, so a known gap **degrading** (graceful-error →
  panic, or panic → silently-wrong compiling output) is CI-visible. Mandate-aligned: snapshots RED, never
  edits an example to go green. **Scope limits:** (1) build it as a **separate** catalog — the existing
  robustness one's contract is "valid/malformed input must *never* panic," and this deliberately blesses
  panics; (2) the harness is **generation-only** (no `cargo check`), so it captures only panic-class gaps,
  **not** compile-class ones (`bool` in a type-choice E0282, a rule named `r` E0574, `x = any`, bare
  `x = int` all generate fine = exit 0, invisible here — those need a negative compile-gate). True
  captured set: top-level fixed-value/null (`answer = 42` / `x = null` → "should not expose Fixed type in
  member" panic), single-field struct map, panic-class control ops.

- **c3 — broaden the no-panic guard. _Conditional; ~M; pair with c2._** Replace the hand-synced 16-entry
  `PRELUDE_TYPES` list in `robustness_tests.rs::all_supported_prelude_types_generate` (its own comment
  admits it's kept in sync by hand) with the matrix's full supported surface (~99 examples), asserting
  each drives the generator without panic/error. Generate-only (weaker than the compile-gate) and overlaps
  c1 on the constructs c1 adds (those get the full snapshot+compile-gate). Best landed *with* c2 as two
  halves of one `catch_unwind` pass (expect-ok + expect-categorized). Needs the same
  `tests/<dir>/<id>.cddl` example-projection as c1 (heterogeneous top-level forms, not a const-array swap).
...
- **Re-bless-snapshot coverage gaps** (each needs a fixture/profile change + snapshot re-bless):
  ...
  generator issue #223).
- **Bless panic-on-valid-CDDL as a tracked-expected fact? (blocks c2.)** The matrix surfaces constructs
  that are *valid CDDL* but make the generator **panic** (top-level fixed-value/null types → "should not
  expose Fixed type in member"; single-field struct map). c2 wants to snapshot those panic outcomes as
  CI-tracked facts — but `input_robustness_catalog`'s standing contract is "input must error gracefully,
  **never** panic" (Findings #1). So: record panic-on-valid-input as *expected* (a separate scorecard
  catalog), or treat each as a generator bug to fix (emit a graceful error instead)? c2 can't land until
  this is called.
