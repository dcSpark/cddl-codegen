...
pub fn coherence_bytes(value: &Holder) -> Vec<u8> {
    crate::generated::serialization::Serialize::to_cbor_bytes(value)
}
...
macro_rules! ref_bytes_codec {
    ($write:ident, $read:ident, $text:literal) => {
        pub fn $write<'se>(serializer: &'se mut cbor_event::se::Serializer, bytes: &[u8], enc: &StringEncoding, canonical: bool) -> cbor_event::Result<&'se mut cbor_event::se::Serializer> { serializer.write_text_sz(core::str::from_utf8(bytes).unwrap(), enc.to_str_len_sz(bytes.len() as u64, canonical)) }
...
macro_rules! complete_codec {
    ($write:ident, $read:ident, $type:ident, $text:literal, $value:expr) => {
        pub fn $write<'se>(serializer: &'se mut cbor_event::se::Serializer, _: &$type, _: bool) -> cbor_event::Result<&'se mut cbor_event::se::Serializer> { serializer.write_text($text) }
