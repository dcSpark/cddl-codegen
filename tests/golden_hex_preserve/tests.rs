...
// from RFC 8949 §3 (integer/length/tag argument forms; §3.2.2-§3.2.3 indefinite-length items),
// NOT copied from generator output and NOT built with the tests/deser_test cbor_event helpers.
// That second independence is the point of this suite: the preserve/canonical fixtures construct
// their expected bytes with the same cbor_event write_*_sz primitives the generated code encodes
// with, so a symmetric bug in that shared layer corrupts expected and actual identically and
// round-trips green. Raw spec-anchored hex is the oracle that catches it.
//
// Each vector asserts decode -> re-encode identity over an IRREGULAR (but valid) encoding:
//   bytes -> T -> bytes must be byte-identical, i.e. the generated code must preserve
//   non-minimal header arguments, indefinite-length framing, chunked strings, and map key order.
//
...
mod golden_hex_preserve {
    ...
    kat_preserve!(pair_minimal, Pair, &[0x82, 0x01, 0x02]);
    ...
    kat_preserve!(pair_arg_1byte, Pair, &[0x82, 0x18, 0x17, 0x18, 0x18]);
    ...
    kat_preserve!(pair_indef_array, Pair, &[0x9f, 0x01, 0x02, 0xff]);
    // element count 2 in a 1-byte argument (0x98 0x02; minimal is 0x82).
    kat_preserve!(pair_array_len_1byte, Pair, &[0x98, 0x02, 0x01, 0x02]);
    ...
    kat_preserve!(kv_minimal, Kv, &[0xa2, 0x61, 0x61, 0x01, 0x61, 0x62, 0x02]);
    ...
    kat_preserve!(seq_indef, SeqHolder, &[0x81, 0x9f, 0x01, 0x02, 0x03, 0xff]);
    ...
    kat_preserve!(tagged_minimal, TaggedOne, &[0xcb, 0x81, 0x05]);
    // tag 11 in a 1-byte argument (0xd8 0x0b).
    kat_preserve!(tagged_tag_arg_1byte, TaggedOne, &[0xd8, 0x0b, 0x81, 0x05]);
