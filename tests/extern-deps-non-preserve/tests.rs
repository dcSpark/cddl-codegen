...
// `_CDDL_CODEGEN_EXTERN_DEPS_DIR_`) under preserve-encodings=FALSE — the sibling of
// `tests/extern-deps` that pins the runtime-flavor contract: a non-preserve generated crate
// constructs `CBORReadLen` via `From<cbor_event::Len>`, so it compiles and round-trips against a
// preserve-flavored common-import crate (the `extern-dep-crate` stand-in for cml_core). The
// mismatched cell (`CBORReadLen::new(Len)` against a `new(LenSz)` runtime) is E0308 without the
// `from(len)` emission this gate guards.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    // preserve-encodings=false maps are plain BTreeMaps (no OrderedHashMap)
    use std::collections::BTreeMap;

    fn mk_everything(extern_foo: ExternCrateFoo) -> Everything {
        let mut foo_map = BTreeMap::new();
        foo_map.insert(
    ...
    fn extern_dep_encodings_thread_across_the_crate_boundary() {
        ...
        // re-encode byte-identically when embedded in a generated struct: the preserve-flavored
        // dep crate's stored encodings have to survive the crate boundary even though the
        // embedding crate itself was generated with preserve-encodings=false
