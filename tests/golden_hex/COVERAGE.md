...
| `f90000` | 0.0 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f90001` | 5.960464477539063e-08 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f90400` | 6.103515625e-05 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f93c00` | 1.0 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f93e00` | 1.5 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f97bff` | 65504.0 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f97c00` | Infinity | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f97e00` | NaN | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f98000` | -0.0 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f9c400` | -4.0 | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `f9fc00` | -Infinity | ➖ | under default flags the generator writes every float as a double (`write_special(Special::Float(..))`), so half-precision is never emitted; the f16 head IS emitted under --preserve-encodings (a recorded head width replays) and under --canonical-form (the shortest lossless width), both covered by the sibling preserve/canonical KATs (tests/golden_hex_preserve, tests/golden_hex_canonical)  [`Special::Float`] |
| `fa47c35000` | 100000.0 | ➖ | under default flags the generator never emits single-precision (same always-a-double reason as float16); the f32 head is covered under --preserve-encodings / --canonical-form by the sibling KATs  [`Special::Float`] |
| `fa7f7fffff` | 3.4028234663852886e+38 | ➖ | under default flags the generator never emits single-precision (same always-a-double reason as float16); the f32 head is covered under --preserve-encodings / --canonical-form by the sibling KATs  [`Special::Float`] |
| `fa7f800000` | Infinity | ➖ | under default flags the generator never emits single-precision (same always-a-double reason as float16); the f32 head is covered under --preserve-encodings / --canonical-form by the sibling KATs  [`Special::Float`] |
| `fa7fc00000` | NaN | ➖ | under default flags the generator never emits single-precision (same always-a-double reason as float16); the f32 head is covered under --preserve-encodings / --canonical-form by the sibling KATs  [`Special::Float`] |
| `faff800000` | -Infinity | ➖ | under default flags the generator never emits single-precision (same always-a-double reason as float16); the f32 head is covered under --preserve-encodings / --canonical-form by the sibling KATs  [`Special::Float`] |
