...
mod tests {
            ...
            for (str_3, str_4) in str_3_encodings.iter().zip(str_4_encodings.iter()) {
                    ...
                    [(false, false), (false, true), (true, false), (true, true)]
                {
                    ...
                    let keys = [
                        ...
                        .collect::<Vec<u8>>(),
                        if has_5 {
                            [cbor_int(5, def_enc), cbor_str_sz("XYZ", str_3.clone())]
                                .into_iter()
                                .flatten()
                                .copied()
                                .collect::<Vec<u8>>()
                        ...
                        [cbor_int(1, def_enc), vec![NULL]]
                            .into_iter()
                            .flatten()
                            .copied()
                            .collect::<Vec<u8>>(),
                        if has_derp {
                            [cbor_str_sz("derp", str_4.clone()), cbor_int(19, def_enc)]
                                .into_iter()
                                .flatten()
                                .copied()
                                .collect::<Vec<u8>>()
                        ...
                        [cbor_str_sz("five", str_4.clone()), cbor_int(5, def_enc)]
                            .into_iter()
                            .flatten()
                            .copied()
                            .collect::<Vec<u8>>(),
    ...
    fn table_arr_members() {
        let non_canonical_bytes = vec![
            ...
            cbor_str_sz(
                "arr2",
                StringLenSz::Indefinite(vec![(2, Sz::Inline), (0, Sz::Inline), (2, Sz::Four)]),
            ),
            vec![ARR_INDEF],
            ...
            cbor_bytes_sz(
                vec![],
                StringLenSz::Indefinite(vec![(0, Sz::Inline), (0, Sz::Four)]),
            ),
            vec![BREAK],
            vec![BREAK],
            ...
            cbor_str_sz(
                "Sixteen",
                StringLenSz::Indefinite(vec![(3, Sz::Inline), (4, Sz::Four)]),
            ),
            cbor_int(4, Sz::Two),
    ...
    fn type_choice() {
        ...
        for str_enc in &str_11_encodings {
            for def_enc in &def_encodings {
                ...
                let irregular_hello_world =
                    TypeChoice::from_cbor_bytes(&irregular_bytes_hello_world).unwrap();
                assert_eq!(
                    irregular_bytes_hello_world,
                    irregular_hello_world.to_cbor_bytes()
                );
                assert_eq!(
                    canonical_bytes_hello_world,
                    irregular_hello_world.to_canonical_cbor_bytes()
                );
                ...
                assert_eq!(
                    canonical_bytes_uint,
                    irregular_uint.to_canonical_cbor_bytes()
                );
                ...
                assert_eq!(
                    canonical_bytes_text,
                    irregular_text.to_canonical_cbor_bytes()
                );
                let irregular_tagged_arr =
                    TypeChoice::from_cbor_bytes(&irregular_bytes_tagged_arr).unwrap();
                assert_eq!(
                    irregular_bytes_tagged_arr,
                    irregular_tagged_arr.to_cbor_bytes()
                );
                assert_eq!(
                    canonical_bytes_tagged_arr,
                    irregular_tagged_arr.to_canonical_cbor_bytes()
                );
            }
        }
    }
    ...
    fn group_choice() {
        ...
        for str_enc in &str_6_encodings {
            for def_enc in &def_encodings {
                let irregular_bytes_3 = vec![arr_sz(1, *def_enc), cbor_int(3, *def_enc)]
                    .into_iter()
                    .flatten()
                    .clone()
                    .collect::<Vec<u8>>();
                let canonical_bytes_3 = vec![arr_def(1), cbor_int(3, Sz::Inline)]
                    .into_iter()
                    .flatten()
                    .clone()
                    .collect::<Vec<u8>>();
                ...
                .clone()
                .collect::<Vec<u8>>();
                let canonical_bytes_tagged_2 =
                    vec![arr_def(1), cbor_tag(10), cbor_int(2, Sz::Inline)]
                        .into_iter()
                        .flatten()
                        .clone()
                        .collect::<Vec<u8>>();
                ...
                let canonical_bytes_foo = vec![
                    ...
                    cbor_string("potato"),
                    cbor_bytes_sz(
                        vec![0xF0, 0x0D, 0xF0, 0x0D, 0xF0, 0x0D],
                        StringLenSz::Len(Sz::Inline),
                    ),
                ]
                .into_iter()
                ...
                let irregular_tagged_2 =
                    GroupChoice::from_cbor_bytes(&irregular_bytes_tagged_2).unwrap();
                assert_eq!(irregular_bytes_tagged_2, irregular_tagged_2.to_cbor_bytes());
                assert_eq!(
                    canonical_bytes_tagged_2,
                    irregular_tagged_2.to_canonical_cbor_bytes()
                );
                ...
                let irregular_inlined =
                    GroupChoice::from_cbor_bytes(&irregular_bytes_inlined).unwrap();
                assert_eq!(irregular_bytes_inlined, irregular_inlined.to_cbor_bytes());
                assert_eq!(
                    canonical_bytes_inlined,
                    irregular_inlined.to_canonical_cbor_bytes()
                );
                ...
                assert_eq!(
                    canonical_bytes_plain,
                    irregular_plain.to_canonical_cbor_bytes()
                );
            }
        }
    }
    ...
    fn cbor_in_cbor() {
        ...
        for def_enc in def_encodings.iter() {
            for str_enc in str_3_encodings.iter() {
                ...
                .clone()
                .collect::<Vec<u8>>();
                let foo_bytes_enc = StringLenSz::Indefinite(vec![
                    (5, Sz::Inline),
                    ((irregular_foo_bytes.len() - 5) as u64, Sz::Eight),
                ]);
                let irregular_bytes = vec![
                    arr_sz(2, *def_enc),
