...
emitted `.d.ts`. This is the only coverage of the schema → `.d.ts` step (a dep bump there is
otherwise invisible to CI — see `tests/TESTING_ROADMAP.md` item 7).
...
`json-ts-types.js` (the wasm-pack `.d.ts` merge step) — see `tests/TESTING_ROADMAP.md` item 7.
