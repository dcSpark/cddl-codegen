// The `--preserve-encodings` FLAVOR of the shipped `static/non_empty_map.rs` (inner map is
// `OrderedHashMap`, keys carry the extra `Hash + Eq` bound) — the substitution
// `generation/export.rs` applies, reproduced here so this hand-written dependency crate
// publishes exactly what a generated preserve-profile dependency would. Consumers of this
// pair generate with `--preserve-encodings=true` for the same reason its wasm wrappers
// speak `OrderedHashMap` across the boundary.
...
impl<K: Ord + core::hash::Hash + Eq, V> TryFrom<Vec<(K, V)>> for NonEmptyMap<K, V> {
    type Error = DeserializeError;
    ...
    fn try_from(entries: Vec<(K, V)>) -> Result<Self, Self::Error> {
        Self::try_from(entries.into_iter().collect::<OrderedHashMap<K, V>>())
    }
}
