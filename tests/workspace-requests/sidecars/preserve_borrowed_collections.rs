...
mod borrowed {
    use wr_dep_wasm::collections::NonEmptyPairMapU64ToIdxFoo;
    use wr_dep_wasm::collections::PairMapU64ToIdxFoo;
}
#[allow(dead_code)]
pub(crate) const BORROWED_SHAPES: &[(&str, &str, &str)] = &[
    (
        "wr_dep",
        "NonEmptyPairMapU64ToIdxFoo",
        "{+ uint => idx_foo} @duplicates preserve",
    ),
    (
        "wr_dep",
        "PairMapU64ToIdxFoo",
