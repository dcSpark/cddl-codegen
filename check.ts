...
function runVerify(o: Opts): Outcome {
  ...
  if (!rust) missing.push("rust `cddl`   build the pinned fork: local-fixes @ 765fd81 in the ~/Documents/git/cddl sibling checkout (or an immutable copy of it), then set RUST_CDDL to it — a stock `cargo install cddl` build is refused by verify.ts's oracle fingerprint");
  if (missing.length) {
