...
- **Tool-owned (rewritten every run).** `package.name`, `package.edition`, `lib.crate-type`, and the
  ...
  `serde-wasm-bindgen`). The dependency set is version-coupled to the emitted code, so the tool must
  keep it in sync — **your edits to these keys are overwritten by design** (the same contract
  `cargo add` makes). A conditional dep is emitted when its flag/type condition holds and *removed*
  when it doesn't, so flipping a flag off (e.g. dropping `--preserve-encodings`) also drops the deps
  it pulled in, rather than stranding them.
