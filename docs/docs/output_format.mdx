...
segment is final or safe-middle; see [wasm differences](wasm_differences#open-struct-map-rest-rows).
...
does so for direct/optional exact fields and exact type-choice payloads (including natural CDDL
`any` arrays), and loose `Vec<[T; N]>` elements when `T` is not `any`. The remaining nested
containing shapes, including nested exact-`any` arrays, are rejected under either JSON flag because
the pinned serde/schemars array derives stop at length 32 and `any` requires its own natural adapter.
This is separate from `bytes .size N`, whose `[u8; N]` storage retains a loose `Vec<u8>` constructor
for compatibility.
