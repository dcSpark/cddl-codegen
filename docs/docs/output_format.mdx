...
| bytes | `{"bytes": "a1b2"}` (written lowercase; this codec's reader is case-INSENSITIVE — see [Hex text](#hex-text-the-canonical-grammar), which contrasts it with the stricter bytes-newtype surface) |
| text | `{"text": "…"}` |
...
Two consumer-facing surfaces render bytes as hex text, and both accept exactly the grammar they
emit: **bare, even-length, lowercase** — no `0x`/`0X` prefix, no uppercase digit, no separators.
...
Both surfaces read through one function, `decode_canonical_hex`, exported from the generated
`serialization.rs` (or from the shared runtime crate under
[`--common-import-override`](./command_line_flags)). Rejections differ only in how much detail the
surface can carry:

| Input | `from_raw_hex` | JSON bytes newtype |
|---|---|---|
| `"a1b2"` | `Ok` | `Ok` |
| `"A1B2"` | `Err` — `invalid character 'A' at position 0` | `Err` — `invalid hex bytes` |
| `"a1B2"` (mixed) | `Err` — `invalid character 'B' at position 2` (the FIRST offending digit) | `Err` — `invalid hex bytes` |
| `"0xa1b2"` | `Err` — `invalid character 'x' at position 1` | `Err` — `invalid hex bytes` |
| `"0Xa1b2"` | `Err` — `invalid character 'X' at position 1` | `Err` — `invalid hex bytes` |
| `"abc"` | `Err` — `odd number of digits` | `Err` — `invalid hex bytes` |
...
outside the canonical alphabet, so the same scan that rejects an uppercase digit names them. The JSON surface is
verdict-only by construction: serde's `invalid_value` carries one wording for every refusal, so a
reader cannot tell which check declined the string.
...
**The `AnyCbor` tagged codec is a separate surface.** Its `{"bytes": "…"}` form is decoded by a
self-contained routine inside the `any_cbor` runtime rather than by the function above, because a
crate emitting `AnyCbor` does not necessarily take the `hex` dependency at all. It writes lowercase
like everything else, but its reader accepts uppercase and mixed case (it still rejects an odd digit
count and any non-hex character, prefix included). Only the two surfaces named at the top of this
section carry the canonical guarantee.
...
whether it compiles. The two hex-reading surfaces — `RawBytesEncoding::from_raw_hex` and the JSON
representation of a `bytes` newtype — used to take uppercase and mixed-case digits and normalize
them to lowercase on the way out. They now accept only the grammar they emit: bare, even-length,
lowercase (full statement: [Hex text](#hex-text-the-canonical-grammar)).
...
peer. Lowercase input, which is everything this crate ever wrote, is unaffected.
...
same one, which is what makes `from_raw_hex(s)?.to_raw_hex() == s` hold for every accepted `s`.
`0x`/`0X`-prefixed input was already rejected and still is, with the same error.
