...
in-scope fixed-value kind is covered — bool, null, uint, nint, text. (A fixed **float** in optional
member position is the one exception: it keeps a pre-existing unbound handling and is unimplemented
under `--preserve-encodings` — tracked by the "OPTIONAL fixed FLOAT member" entry in
`cddl-matrix/ROADMAP.md`'s findings ledger and the `preserve_encodings_supports_floats` stub gate.)
