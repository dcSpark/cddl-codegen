...
The **wasm crate gets the same glue for `_CDDL_CODEGEN_EXTERN_TYPE_`**. Its generated code names the
...
the generated boundary code calls). A `_CDDL_CODEGEN_RAW_BYTES_TYPE_` declared in the crate's own spec
gets **no wasm-side glue yet** — its generated wasm code still names the wrapper by bare ident, so a
crate-root wasm wrapper for it does not resolve (a known limitation; tracked by the
obsolete-fixture-contract residual in `tests/TESTING_ROADMAP.md` § Standing-system residuals).
Raw-bytes types imported from a *dependency* crate are unaffected — they resolve through the
dependency's import path, not this glue.
