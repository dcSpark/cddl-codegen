...
**Encoding-operation descent does not.** A position strictly inside a tag or a `.cbor` unwrap is not the member's type, and is spelled from the type actually at that point. `cred_bytes = bytes .cbor credential` emits `pub type CredBytes = Credential;` and reads its payload with `Credential::deserialize` — *inside* the already-unwrapped byte string:
...
`CredBytes::deserialize` there would compile (the alias is transparent) and would lie: it claims to read a bytes-wrapped value at the position that reads the payload. The carve-out is deliberate and the inconsistency it keeps is the correct output — this is the one place the rule and "one spelling per member" disagree, and the rule loses on purpose. A container inner *inside* such a payload is unaffected: it is a member position of its own again, so `bytes .cbor [* stake_credential]` reads its elements with `StakeCredential::deserialize`.
