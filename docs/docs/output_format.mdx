...
is unconditionally `#![no_std]`), so there is nothing to forward to.
...
The sharpest instance is hand code that **boxes a dependency's error type**. `hex::FromHexError`'s
`Error` impl is gated on hex's own `std` feature, so `Box::new(e)` into
`DeserializeFailure::InvalidStructure` stops compiling once `hex` is in alloc mode. The fix is a
local newtype, not re-enabling the dependency's defaults — `tests/custom_serialization` in this
repository is the worked exemplar:
...
`HexDecodeError(..)` layer into every rendered message. The wrapper stays invisible in output.
...
pulls `hex` (or serde/serde_json/schemars) with default features turns their `std` feature back on
for everyone, and the boxing keeps compiling. This is not hypothetical — it happens inside this
repository: the preserve fixture takes the `cddl` oracle as a test dependency, which pulls `hex`
with default features, and its identical `Box::new(FromHexError)` compiled green while the core
fixture's — same line, no such dependency — failed. Check the crate built standalone, which is
exactly what the [`no-std-check` shim](#the-no-std-check-shim-crate) does with its empty
`[workspace]`.
