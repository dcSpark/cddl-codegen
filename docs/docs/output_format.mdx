...
`opt_fixed_member_float`). Under `--preserve-encodings` the member additionally carries its head-width
encoding variable, so an absent member stores no width and a present one re-emits at the width it
arrived on. A fixed float value is a literal, not a prelude name, so it declares no head set and accepts the literal at any width — the per-name head sets are the separate rule in [Floats](current_capacities#types), and the canonical narrowing is under [`--canonical-form`](command_line_flags).
...
`hashlink` and the pinned `cbor_event` fork never appear: neither declares a `std` feature (the fork
is unconditionally `#![no_std]`), so there is nothing to forward to. An entry names the
