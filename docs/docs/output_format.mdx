...
- **Tool-owned: `src/generated/**` (clobbered every run).** All generated code lives here —
  ...
  overwritten wholesale on every export — never hand-edit the *code*. One exception: **own-line
  comments you add to a generated `.rs` are carried across regeneration** by symbol identity (which
  named item they sit in/above) — a comment on an unchanged item transfers, and a comment whose
  anchor changed too much becomes a `compile_error!` block (loud, never a silent drop) so you review
  it. Outside those tagged blocks this never affects the generated code tokens; pass
  `--no-preserve-comments` to turn it off and clobber pristine. Two carve-outs: trailing
  (end-of-line) comments are not carried in v1 — move them to their own line — and `///` doc text
  on tool-documented items is tool-owned (it flows from your CDDL/`@doc`; hand edits to it are
  dropped, so edit the CDDL instead).
