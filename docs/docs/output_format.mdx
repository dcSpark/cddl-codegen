...
Because the invalid state is unrepresentable, a struct whose only source of fallibility *was* an
occurrence bound gets an **infallible `new()`**: every argument is already valid by construction, and
serialization stays infallible (there is no empty value to fail on). Construction that used to check a
length now hands over an already-checked value.
