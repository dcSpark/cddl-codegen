...
**The `std` feature does not forward to dependency features.** It is `std = []`, an in-crate `cfg`
only, and the crate's dependencies stay in alloc mode permanently: `hex` with
`default-features = false, features = ["alloc"]`, and `serde` / `serde_json` / `schemars` likewise
`default-features = false` with only the features the emitted code needs. Forwarding
(`std = ["hex/std", …]`) is not available: those dependencies are flag- and type-conditional, and a
feature referencing a dependency that this flag set did not emit is a cargo manifest error. If your
hand code wants a dependency's `std` features, add them to that dependency's entry — the
[manifest merge](#generated-cargotoml-merge-not-clobber) unions features and keeps yours.

**Scope is the rust crate.** The `wasm` and `wasm/json-gen` crates stay `std` by nature
(`wasm-bindgen`, and writing schema files to disk); they consume the rust crate with default
features on and are unaffected. The [`--export-static-crate`](./command_line_flags) target declares
the same `std` feature — it receives `ordered_hash_map.rs`, whose `cfg` would otherwise silently
select the `no_std` hasher there, since an undeclared feature is always false — but that manifest's
dependency specs are co-owned and deliberately not reshaped, so the feature is an in-crate switch
and does not by itself make that crate buildable with `--no-default-features`.
...
unmodified crate is ours; otherwise it attributes to hand-written additions. Calibrate on one detail:
...
**7. The [`--export-static-crate`](./command_line_flags) target gains a `std` feature too**, and it
means less than it looks like: that manifest is co-owned, so its dependency specs are deliberately
left alone. Declaring the feature is what keeps the exported `ordered_hash_map.rs` on `RandomState`
instead of silently selecting the `no_std` hasher; it does not by itself make that crate buildable
...
- **Tool-owned (rewritten every run).** `package.name`, `package.edition`, `lib.crate-type`, and the
  ...
  **dropped** from your entry rather than preserved: a stale version requirement would constrain
  ...
  and dropping `--wasm` removes exactly that key. `std = []` is written on every run whatever the
  flags (`default = ["std"]` is its own case, below). Because each op addresses a single
...
- **Seeded once.** `package.version` is written only if it's absent. After the initial `0.1.0`
  ...
  regenerations.
- **Merged, not replaced.** `features.default` — the tool's entries are added to whatever list is
  already there rather than overwriting it: your entries keep their order (and come first) and the
  tool's are appended. The consequence worth knowing before you meet it is that the merge is one-way
  — an entry the tool asserts cannot be durably deleted from this list, because the next
  regeneration adds it back.
