...
- **Tool-owned: `src/generated/**` (clobbered every run).** All generated code lives here —
  ...
  it. This never affects the generated code tokens; pass `--no-preserve-comments` to turn it off and
  clobber pristine. Trailing (end-of-line) comments are not carried in v1 — move them to their own
  line to preserve them.
