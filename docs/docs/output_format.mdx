...
**Unused collection-type imports are pruned.** A generated file imports the collection helper types (`BTreeMap`, `OrderedHashMap`, `NonEmptyVec`, `NonEmptyMap`, `OrderedSet`, `NonEmptyOrderedSet`) and, under `--preserve-encodings`, the encoding enums (`LenEncoding`, `StringEncoding`, `TagPresenceEncoding`) only when the file — or one of its descendant modules — actually names them: a usage-derived post-pass reads the rendered source and drops any of those nine imports the file's module family references nowhere, so a module that uses no maps/non-empty collections/reject-duplicates sets/encodings carries no unused-import warning for them, independent of what sibling modules use. (The two `OrderedSet` twins ride the spec-global `@duplicates reject` gate — see [Reject-duplicates containers](#reject-duplicates-containers-uniqueness-twins) — so a scope with a reject-set rule pushes them into *every* scope's `mod.rs`, and this pass is what keeps the scopes that never name them warning-free.) Descendants count because a child module's `use super::*;` re-exports the parent module's private imports — the `BTreeMap` in a scope's `mod.rs` can be the one its `serialization.rs` relies on. A descendant keeps a parent's copy only when it would genuinely resolve the name through the parent, though: one that carries its own direct import of the same name, or that sits at/under the very module the parent imports it from, resolves the name locally and does not hold the parent's import alive — which is what keeps a `--preserve-encodings` `mod.rs` free of dead `LenEncoding`/`StringEncoding` imports whose only consumers are `cbor_encodings.rs` (own import) and `serialization.rs` (the defining module). Only private `use` items are candidates (`pub use` re-exports are API surface and never touched). This is scoped to those concrete types by design — trait imports (e.g. `cbor_event::se::Serialize`, exercised via a method call whose name never mentions `Serialize`), macro imports, and glob imports (`use ...::*;`) cannot be proven unused by name and are left as-is, so a few of those may still be broader than strictly needed.

**Re-export-only files carry no import walls at all.** A generated file whose entire body is extern re-export glue — every top-level item a `use`, with only `crate::`-anchored `pub use` re-exports (the `mod.rs` an extern-only CDDL scope produces) — has *all* of its private `use` items dropped, not just the seven concrete types above. Such a file's shape proves nothing can consume a private import (no local code, and no descendant module whose `use super::*;` could re-export one), so the wider rule is sound even for globs and trait imports the name-based prune must otherwise keep. The result is a re-export-only `mod.rs` reduced to its header comment and its `pub use crate::…;` lines, with zero `unused_import` warnings.
...
## Transparent tag-set collections

The recognized tagged-or-untagged set idiom (`my_set = #6.258([* a]) / [* a]` — see
[Current capacities](current_capacities) § "Transparent tag-set idiom") emits the SAME output a
bare `#6.N([* a])` array rule gets: a transparent alias (`pub type MySet = Vec<a>;`, or
`NonEmptyVec<a>` for the `[+]` flavor with the full non-empty contract below), never an enum or a
dedicated set struct. Under `--preserve-encodings` the tag's wire presence rides a
`*_tag_encoding: TagPresenceEncoding` field in the **holder's** encoding struct — a tri-state
(`Tagged(Option<Sz>)` / `Untagged`, default `Tagged(None)` = tagged at fit-minimal size) that is
deliberately NOT `Option<Sz>`, which would conflate "untagged" with "tagged, default size". Like
every collection encoding detail, it is positional (holder-side) rather than value-carried: a
value moved to a different holder re-serializes with the defaults. `--canonical-form` normalizes
the tag's size, never its presence.
...
A [`@duplicates reject`](./comment_dsl) directive on an array-shaped collection rule — a plain `[* T]`
/ `[+ T]`, or either arm of the transparent tag-set idiom (`#6.258([* a]) / [* a]`) — swaps the
transparent-alias target from the bare `Vec`/`NonEmptyVec` to a **uniqueness twin** that makes a
duplicate element unrepresentable, the same enforcement-by-type move the non-empty containers make for
the length bound:
...
- **Checked `push` returns `Result`.** Appending an element already present is refused with
  ...
  must be fallible). `NonEmptyOrderedSet` additionally gives checked `pop`/`remove` (refused at
  length 1) exactly like `NonEmptyVec`.
