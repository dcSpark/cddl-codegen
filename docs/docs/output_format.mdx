...
The wasm wrapper and JSON/schema surfaces for `any` are not yet available; a spec whose types contain
`any` is [rejected at generation time](current_capacities#the-any-type-loose-cbor) under `--wasm=true`
or the JSON flags, with a message naming the workaround. Those surfaces arrive in a later phase.
