...
- **A member reached through an alias carrying a [`@custom_serialize`/`@custom_deserialize` pair](comment_dsl.mdx#custom_serialize--custom_deserialize).** Such an alias mints no Rust type — a `pub type` there would carry the aliased type's built-in codec as a standalone wire contradicting the one every embed site writes — so there is no ident for a member to keep, and the member spells the type the alias **resolves to** (`inner = uint ; @custom_serialize …` gives `pub a: u64`, and an alias of a marker gives `pub p: PolicyId`). The alias name still governs everything derived from the *name* rather than the type: enum variant idents, the structural wasm class of a collection over it (`InnerList`), and the encoding-sidecar shape. The suppressed alias was transparent, so the resolved spelling and the vanished one always denoted one type — no wire byte, `Ord`/`Hash` behaviour or JSON face changes with it.
...
element type like `* 5`, or a **plain group** element type) are graceful rejections naming the
supported form. The plain-group case (`kv = (a: uint, b: uint)`, `t = [ c: uint, * kv ]`) refuses
