...
check_schema_ref_closure;` always, and `use <lib>::json_schema_gen::add_schema;` when this crate has
registration rows of its own — so a workspace of several generated packages shares one
implementation rather than one copy per json-gen crate. The module is `rust/src/generated/
