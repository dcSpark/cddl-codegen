...
**Unused-variable warnings are held to the same standard.** An emitted binding whose body never reads it is spelled to say so — an unread loop index binds `_i` (e.g. a [`@duplicates preserve`](comment_dsl#duplicates) pair-map whose entries carry no per-entry encoding state to index), a constant-count match arm binds `_` — so a generated crate contributes no `unused_variables` noise to a consumer's build. Both classes (unused imports beyond the documented trait residue above, and unused variables) are enforced by warning scans over every generated crate the tool's own test corpus compiles.
...
- **C-style enums** — multi-value all-fixed choices (`foo = 0 / 1 / 2`) are encoded *inline wherever they are used*, not via an `impl` on the enum. The exception is a mandatory tag directly on the choice rule under `--preserve-encodings`: that rule uses the data-carrying union shape below so the enum can retain the tag head's width. A **one-value named rule** (`answer = 42` or `magic = h'CAFE'`, including a tagged or `bytes .cbor` spelling) is different: it is a nominal singleton TypeChoice with direct codecs, precisely so its standalone API has the declared wire shape.
...
- **A member reached through an alias carrying a [`@custom_serialize`/`@custom_deserialize` pair](comment_dsl.mdx#custom_serialize--custom_deserialize).** Such an alias mints no Rust type ([why](comment_dsl.mdx#reaching-an-annotated-alias-through-another-rule)), so there is no ident for a member to keep: the member spells the type the alias **resolves to** (`inner = uint ; @custom_serialize …` gives `pub a: u64`, and an alias of a marker gives `pub p: PolicyId`). The alias name still governs everything derived from the *name* rather than the type — enum variant idents, the structural wasm class of a collection over it (`InnerList`), and the encoding-sidecar shape — and no wire byte, `Ord`/`Hash` behaviour or JSON face changes with it.
...
entry count — so the row has no wire. The guard reads the *resolved* slot type, so the bare, alias and
tagged (`* uint => #6.10(kv)`) spellings refuse alike, and both slots are reported when both offend.
The remedy the message names is the group's array framing, which gives the slot the one item it needs:
`t = { c: uint, * [kv] => uint }` / `t = { c: uint, * uint => [kv] }`; a tag belongs *outside* that
framing, on the framed reference (`#6.10([kv])`, not `[#6.10(kv)]`). The prefix-less spelling
...
A CDDL array that mixes **fixed members with a trailing `* t` or `+ t` element** — `[uint, tstr, * uint]` or `[uint, + uint]` — is
...
element and follow ≥1 fixed member. `*` is loose; `+` and `1*` are the restricted min-one form.
Other spellings (a bounded occurrence, a non-final tail, a rest tail inside a group-choice arm or inside a plain group's body,
a fixed-value element type like `* 5`, or a **plain-group element type**) are graceful rejections
naming the supported form. The plain-group element type
(`kv = (a: uint, b: uint)`, `t = [ c: uint, * kv ]`) refuses
because a rest tail collects one value per remaining array element and a plain group is not one —
it has no type of its own, splicing its members flat instead — and the guard reads the *resolved*
element type, so the bare, alias (`* kv_alias`) and tagged (`* #6.10(kv)`) spellings refuse alike.
The remedy the message names is the group's array framing: `w = [kv]`, then `t = [ c: uint, * w ]`;
a tag belongs on that framed reference (`* #6.10(w)`) rather than on the group. A single
**mandatory** splice (`t = [ c: uint, kv ]`) and the sole-element homogeneous array (`[* kv]`) stay
supported; the full splice-position matrix is
[Plain group placement](current_capacities#plain-group-placement).

**The `rest` field.** A loose `*` tail is `pub Vec<T>`, excluded from `new()` and defaulting empty.
A `+` / `1*` tail is `pub NonEmptyVec<T>`: `new()` takes its first tail element, and decoding stages
into `Vec<T>` before entering the same `NonEmptyVec::try_from` door. Empty definite and indefinite
tails therefore reject with the stable min-one RangeCheck.
...
invariant. A restricted `+` / `1*` tail is required with `minItems: 1` and enters the same non-empty
door on JSON read. An `any`-element tail renders **natural-fallible** (the same
...
substitute). The JSON Schema is the ordinary array shape, with `minItems: 1` for the restricted
form.

**wasm.** The rest field is exposed as a **read-only getter named after the field** (`rest()` by
default) returning the minted list wrapper. The loose form has no constructor argument; the
restricted form takes its first tail element and returns a restricted non-empty wrapper;
see [wasm differences](wasm_differences#open-struct-map-rest-rows).
...
homogeneous ARRAY window except bare `*` is likewise type-enforced: `[2*5 T]`, `[*5 T]`, `[2* T]`,
`[? T]`, and `[2*2 T]` emit
`BoundedVec<T, MIN, MAX>` (`u64::MAX` is the target-independent unbounded carrier). Bare `[* T]` remains `Vec<T>`.

A bounded `@duplicates reject` array deliberately remains the compound-constraint residue: it uses
`OrderedSet<T>` (or `NonEmptyOrderedSet<T>` for `[+]`) and retains its inclusive cardinality check at
the API and decode boundaries. There is no `BoundedOrderedSet` yet, so a reject set is never silently
lowered to `BoundedVec`.
...
`minProperties`/`maxProperties`. Bounded `@duplicates preserve` tables are refused until a bounded
pair-map carrier exists.
...
A table rule that [owns an encoding operation](#type-spelling-at-member-positions) — a tagged body
(`#6.n({* k => v})`, the optional-tag idiom included) or a `@newtype` rule — mints a **wrapper
struct** whose stored inner is the same twin (`pub struct TaggedMeta(pub(crate) PairMap<u64,
Vec<u8>>)`), so the policy selects the representation while the wrapper's codec owns the tag.
...
One sharp edge of that demanded set, worth knowing before you write a wrapper: it is derived from
every in-crate marker rule, not from what the wasm face actually references. A raw-bytes or extern
type reached **only as a generic argument** (`ext_set<pub_key>`, where the wasm code names only the
concrete `ExtSetPubKey` wrapper) is still demanded and re-exported, so the wrapper you dutifully
supply earns an `unused import` warning at the generated re-export line. Supplying it is harmless
beyond that warning; if you would rather not maintain a dead wrapper, delete that one glue line via
the [`cddl-codegen:replace`](./preserving_edits) route above and the requirement disappears with it.
...
encodes the container for the two [`@duplicates`](./comment_dsl#duplicates) twins
...
workspace regen passes [`--key-requests`](./command_line_flags) beside `--wrapper-requests`; with
...
provisions `BoundedVec`; either may emit its loose `try_from` source wrapper as an (unattributed)
...
- **Tool-owned (rewritten every run).** `package.name`, `package.edition`, `lib.crate-type` (whose
  value tracks the faces this run emits — see the [`--component` flag](./command_line_flags)), and the
