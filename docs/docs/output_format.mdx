...
report it — and otherwise it attributes to hand-written additions. Calibrate on one detail: that gate
runs in the repo's `local` and `full` tiers, which are a run-before-shipping discipline rather than a
continuous one (its CI runs the `fast` tier only).
