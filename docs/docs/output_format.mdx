...
The generated **rust** crate is `no_std`-capable: it declares a default-on `std` cargo feature, and a
`no_std` consumer depends on it with `default-features = false`. Everything under `src/generated/**`
is generated `no_std`-clean. The crate root `src/lib.rs`, however, is
