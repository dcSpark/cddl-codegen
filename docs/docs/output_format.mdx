...
`--workspace-dep` is set (same empty-but-present semantics as `borrowed_collections.rs`). Note:
generation must currently also run with `--wasm` — under `--wasm=false` the `--workspace-dep` flag
(this sidecar included) is presently not applied. Where the
