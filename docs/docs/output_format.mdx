...
A key domain outside `uint`/`text`/`any` is CBOR-only for now: it is rejected under
[`--json-serde-derives`/`--json-schema-export`](command_line_flags), because the flattened-JSON
surface below images rest keys as JSON object member names and that image is defined only for the
three fast-path domains.
