...
(`Foo = _CDDL_CODEGEN_EXTERN_TYPE_`) requires you to supply its Rust definition; wire it in from the
thin root and it survives every regeneration:
...
pub use utils::Helper;    // re-export it at the crate root
```

```rust
// src/utils.rs — references generated types through the crate-root glob
use crate::Foo;
pub struct Helper;
pub fn round_trip(f: Foo) -> Foo { f }
```
