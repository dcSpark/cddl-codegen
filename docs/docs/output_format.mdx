...
the absence of a field-local custom codec, an effective wire head on both boundary items, and no CBOR
major type in common with the repeated element. A generator-proven head qualifies; a transparent
custom-codec alias may instead declare the head with
[`@custom_wire_major`](comment_dsl#declaring-the-wires-major-type-custom_wire_major).
...
declared custom head still has to be disjoint from the other effective boundary set; same-major
value discrimination is not inferred. The declaration is not consumed by a final tail, an exact
