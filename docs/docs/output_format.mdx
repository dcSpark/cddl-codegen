...
On the **wasm** read side the two `Option`s are flattened into one, because `wasm_bindgen` cannot
...
Other newtype wrappers (bare tags and unconstrained [`@newtype`](./comment_dsl) rules) and the wasm
