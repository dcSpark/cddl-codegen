...
An exact-zero fixed member (`0*0 key: value` or `*0 key: value`) is a different constraint: the
fixed key is forbidden, not an optional field. It emits no value field or schema property. When such
a record has a captured map row, `rest` is private even for a loose row; `new` accepts the complete
carrier and returns `Result`, `rest()` lends it immutably, and `insert_rest` validates each key before
changing the carrier. This prevents an otherwise-public loose map from constructing an invalid record.
...
**Current limitation.** A native public `rest` mutation or the wasm `insert_<row>` door can currently
put a captured key equal to a declared **CBOR** key into the same record. Serialization then writes
that key twice and the generated decoder rejects its own output on reread. This is tracked as
`testing.open-rest-declared-cbor-key-collision-reject-before-write`; do not rely on such a value
being serializable.
...
differences](wasm_differences#open-struct-map-rest-rows).
...
typed entries**. Its public `entries` field is the actual restricted carrier — `NonEmptyMap<K_t,
V_t>`, or `NonEmptyPairMap<K_t, V_t>` under `@duplicates preserve` — so clearing or removing its final
