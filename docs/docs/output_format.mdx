...
(which the generator treats as fatal). The skip freezes exactly the bytes rustfmt would otherwise
produce, so it is removable once the upstream fix (PR #5708) ships and reaches consumers.
