...
- the same sidecars for an [open struct-map rest row](#open-struct-maps-rest-rows) or [open array occurrence segment](#open-arrays-rest-tails);
...
[open struct-map](#open-struct-maps-rest-rows) rest row — structurally simpler because arrays are
...
mandatory and expands to exactly one CBOR item. A variable-cardinality middle window also needs no
field-local custom codec, a generator-proven wire head on both boundary items, and no CBOR major type
in common with the repeated element. An exact window (`N*N`, except `1*1` which is an ordinary field)
...
back to that value. A variable safe-middle loop greedily consumes only the repeated element's majors
and stops at the major-disjoint suffix; an exact loop consumes its declared count and then reads that
same suffix. Neither seeks, rewinds, or speculatively parses the suffix. Variable same-major/value-
...
`* 5`, and a **plain-group element type** remain graceful rejections naming a supported remedy. The
...
type, so the bare, alias (`* kv_alias`) and tagged (`* #6.10(kv)`) spellings refuse alike. The remedy
the message names is the group's array framing: `w = [kv]`, then `t = [ c: uint, * w ]`; a tag belongs
on that framed reference (`* #6.10(w)`) rather than on the group. A single **mandatory** splice
(`t = [ c: uint, kv ]`) and the sole-element homogeneous array (`[* kv]`) stay supported; the full
splice-position matrix is [Plain group placement](current_capacities#plain-group-placement).
...
preserving that public ABI. Every other window is `pub BoundedVec<T, MIN, MAX>`: `new()` takes the
**complete checked carrier**, including zero-minimum and exact-zero windows, so callers can construct
every admitted value without a bypassable `Vec` door. Decode stages one loose `Vec<T>` then enters
exactly one `TryFrom` door for either restricted carrier; definite and indefinite values below or
above the inclusive window reject with its `RangeCheck` for a final tail. A middle segment stops
...
framing supplies the proof. A final optional with no following item remains owner-length-delimited.
...
`BoundedVec` before native construction; its public carrier does not depend on whether the segment is
final or safe-middle; see [wasm differences](wasm_differences#open-struct-map-rest-rows).

### The `@ignore` (tolerate-and-drop) flavor

Marking the occurrence entry [`@ignore`](comment_dsl#ignore) switches from capture to
