...
snapshot. A loose row has no `new()` argument and defaults empty; a restricted row takes its complete
checked wrapper at the fallible constructor boundary. Exact-zero records instead take the complete
wrapper at their fallible constructor boundary even for a loose row, so its forbidden-key check cannot
be bypassed. Neither flavor has a setter; see [wasm
