...
arrived on — see [`--preserve-encodings`](command_line_flags) for the float width rules.
...
The degenerate case of T1 is worth stating outright: a `K_t` that admits **every** string — a bare
`text` typed row — takes every member name on read, so the catch-all region is unreachable through
JSON. Open tables are for a typed row whose admissible names are a proper subset (a fixed-width hex
image, a decimal domain); a `text` typed row is a CBOR-only shape.
