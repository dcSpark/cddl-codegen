...
- **Asserted, never removed.** The `[dependencies]` entries named by
  [`--json-gen-dep`](./command_line_flags) in `wasm/json-gen/Cargo.toml` (and, outside the generated
  ...
  For `--json-gen-dep` the package name exists only inside the flag value, so a run without the flag
