...
Under the JSON flags `AnyCbor` derives serde like any other type, but its JSON is a **representation of
CBOR, not natural JSON**: every value is a single-key object keyed by the snake_case kind name. JSON is
the deliberately lossy side of the contract — encoding detail never appears, so two preserve values that
differ only in encoding render identically.
...
table (or any table with a non-string key) cannot serialize to JSON — serde_json requires string map
keys, so `to_json` errors at runtime, matching how a `{ * bytes => uint }` table already behaves;
string/uint-keyed `any` ranges (`{ * uint => any }`) are unaffected.
