...
- The JSON Schema emits `properties` (the declared members) **plus `additionalProperties`** (the rest
  range's schema; permissive for an `any` range) — the honest open-map shape (json2ts → a TS interface
