...
mandatory, expands to exactly one CBOR item, has no field-local custom codec, and has no CBOR major
type in common with the repeated element. Neither boundary item may have a custom- or extern-owned,
otherwise-unproven wire head. Bare `*` / `0*` is loose; `+` / `1*` is the compatibility
min-one form; every other window (`2*3`, `*3`, `2*`, `*0`, `0*0`, …) is bounded.
...
back to that value. The safe middle loop greedily consumes only the repeated element's majors and
stops at the major-disjoint suffix; it never seeks, rewinds, or speculatively parses the suffix.
Same-major/value-discriminator designs, multiple occurrences, inline-group occurrences,
