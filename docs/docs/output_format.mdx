...
Like `borrowed_collections.rs`, this file's rows carry **no per-row comments** — all commentary lives in
the fixed banner, and each row's type path is its own traceability. A comment on a row is a trap: a spec
change can delete any row, and the edit-preservation overlay turns a comment stranded on a deleted row into
a `compile_error!` sentinel that every later regen carries forward. `key_demand_assertions.rs` (its
`_demand_<rule>` fns, one per `@used_as_key` tag) is banner-only for the same reason.
