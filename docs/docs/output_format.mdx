...
element type like `* 5`) are graceful rejections naming the supported form. One element-type
restriction is not yet graceful: a rest tail whose element is a **plain group**
(`kv = (a: uint, b: uint)`, `t = [ c: uint, * kv ]`) currently **aborts** generation rather than
rejecting — name the group's array framing instead (`w = [kv]`, `t = [ c: uint, * w ]`), which is
supported.
