...
A CDDL array that mixes **fixed members with a trailing occurrence-bearing element** — `[uint, tstr, * uint]`, `[uint, + uint]`, or `[uint, 2*3 bytes]` — is
...
element and follow ≥1 fixed member. Bare `*` / `0*` is loose; `+` / `1*` is the compatibility
min-one form; every other final window (`2*3`, `*3`, `2*`, `*0`, `0*0`, …) is bounded. Non-final
or multiple tails, a rest tail inside a group-choice arm or inside a plain group's body, a fixed-value
element type like `* 5`, or a **plain-group element type** are graceful rejections
...
**The `rest` field.** A loose `*` / `0*` tail is `pub Vec<T>`, excluded from `new()` and defaulting
empty. A `+` / `1*` tail is `pub NonEmptyVec<T>`: `new()` takes its first tail element, preserving
that public ABI. Every other final window is `pub BoundedVec<T, MIN, MAX>`: `new()` takes the
**complete checked carrier**, including zero-minimum and exact-zero windows, so callers can construct
every admitted value without a bypassable `Vec` door. Decode stages one loose `Vec<T>` tail then
enters exactly one `TryFrom` door for either restricted carrier; definite and indefinite values below
or above the inclusive window reject with its `RangeCheck`.
...
rendering. Only the loose tail *writes* by **skipping the field when empty** and *reads* by
**defaulting it to empty when absent** — so empty loose tail ≡ closed-struct JSON. Restricted tails
are required: `+` / `1*` publishes `minItems: 1`; bounded tails publish their applicable `minItems`
and `maxItems`, including zero; all enter their same checked door on JSON read. An `any`-element tail
renders **natural-fallible** (the same
...
min-one form retains its first-tail-element constructor; each bounded form takes its matching checked
bounded list wrapper. WIT despecializes that wrapper to `list<T>` and component glue fallsibly
converts it back to `BoundedVec` before native construction; see
[wasm differences](wasm_differences#open-struct-map-rest-rows).
...
`@ignore` is deliberately limited to loose `*` / `0*` tails. Dropping a restricted tail would
