...
- a **wrapper rule's inner value** — the type a tag-head rule (`tagged_sc = #6.9(stake_credential)`) or a `@newtype` rule wraps — and the `new` / getter / `From` signatures built from it;
...
**An alias that names a wrapped form spells its payload from the target.** When the alias *rule itself* carries the tag or `.cbor`, the alias ident denotes the wrapped value, not the payload — so the payload read names the target:
...
`cred_bytes` emits `pub type CredBytes = Credential;` and reads its payload with `Credential::deserialize`, *inside* the already-unwrapped byte string. `CredBytes::deserialize` there would compile (the alias is transparent) and would lie: `CredBytes` **is** the bytes-wrapped thing, so naming it at the position that reads the payload claims to read a bytes-wrapped value. This is the one place the rule and "one spelling per member" disagree, and the rule loses on purpose.

What matters is which declaration owns the encoding operation, not that there is one. An operation written into the **member's own type expression** leaves the alias denoting exactly the value being read, so the spelling survives it: `f: #6.9(stake_credential)` is a field typed `StakeCredential` read with `StakeCredential::deserialize` inside the tag, and `j: bytes .cbor stake_credential` is a field typed `StakeCredential` read with `StakeCredential::deserialize` inside the byte string — the operator is the same one `cred_bytes` seals on, and the ownership is what differs. Stacking the two resolves on the rule's account — `f: #6.9(cred_bytes)` still reads `Credential::deserialize`, because it is `cred_bytes` that names a wrapped form. And a container inner *inside* a payload is a member position of its own again, so `bytes .cbor [* stake_credential]` reads its elements with `StakeCredential::deserialize`.
