...
Wherever generated code **declares** the type of a member, it spells that type **as you declared it** — keeping the outermost alias ident, with no resolution to the alias's structural target. Given

```cddl
policy_id = bytes
epoch = uint
holder = [pid: policy_id, m: {* epoch => text}]
```

the emitted `pid` is `PolicyId`, never `Vec<u8>`, and `m`'s encoding sidecar is keyed by `Epoch`, never by `u64`.

The declaration positions this covers:
...
- **Member deserialize call targets.** A member typed `delta_coin = int` is still read with `Int::deserialize`, and one typed `stake_credential = credential` with `Credential::deserialize` — the alias's structural target, not the alias. Those are call expressions rather than type declarations; nothing about the emitted bytes or the public type surface depends on which spelling appears there.
