...
Map literals also generate a type for them with `new()`, `len() -> usize`, `insert(K, V) -> Option<V>`, `get(K) -> Option<V>` and `keys() -> Vec<K>`. The table type will have a `MapKeyToValue` name for whichever `Key` and `Value` types it's exposed as if it's anonymously inlined as a member. A named table rule takes on its identifier — unless a `MapKeyToValue` wrapper for the same map shape was already emitted for an anonymously-inlined or resolved-inline use (e.g. as the inner of a `@newtype` wrapper), in which case the named rule reuses that shared wrapper and its identifier survives only as a rust-level `pub type` alias in the wasm crate (`wasm_bindgen` does not export type aliases, so the JS class is the `MapKeyToValue` name in that case).
