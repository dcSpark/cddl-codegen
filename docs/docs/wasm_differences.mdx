...
struct. On the rust side the wrapper is fully usable — `new(inner)` plus `From` conversions in both
directions. On the wasm side the wrapper currently exposes **only** `to_cbor_bytes()` /
`from_cbor_bytes()`: no constructor taking the inner value and no getter reading it back, so from
JavaScript the wrapped content can only be produced or inspected as CBOR bytes (for a plain `@newtype`
the wrapper's bytes are exactly the inner value's bytes; for a tag they include the tag header). Two
shapes avoid the limitation: a wrapper over a named array/map inherits that collection's wasm API
(`new`/`add`/`insert`/`get`), and a tag over a struct needs no wrapper at all — the tag is folded into
the struct's own serialization, so the struct keeps its normal constructor and getters.
