...
Text arrays also get a list wrapper, but on different grounds: `wasm_bindgen` can expose
`Vec<String>` directly (strings are **copied** at the boundary, so the ownership hazard above
doesn't apply to them). The wrapper is kept there for API uniformity — every non-number-array
collection is a `*List` class — and because switching a generated signature from a class to a bare
JS array is a breaking change for downstream consumers.
