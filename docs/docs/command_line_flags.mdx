...
**Only path dependencies need naming.** The tool's own third-party dependencies forward automatically wherever they can — `serde`, `serde_json`, `schemars` and `hex`, the ones it ships in alloc mode and which have a `std` feature to name. `hashlink` and the pinned `cbor_event` fork have none, so nothing forwards to them. See [Output format § The `std` feature](./output_format#the-std-feature-building-without-std) for the computed list and its merge rules.
