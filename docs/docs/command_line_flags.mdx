...
For **floats**, "minimal-length" means the shortest of the three float head widths (`0xf9`/`0xfa`/`0xfb`, RFC 8949 §3.3) that still represents the value exactly, so `1.0` canonicalizes to `0xf9 0x3c00` from any width while `1.1` stays a double. NaN is the one case where canonicalizing changes the VALUE and not just the head: any NaN, carrying any payload, at any width, canonicalizes to the zero-payload quiet NaN `0xf9 0x7e00` (RFC 8949 §4.2.2). A non-canonical write preserves the payload instead, so a payload-carrying NaN round-trips byte-exactly through `to_cbor_bytes()` and collapses only through `to_canonical_cbor_bytes()`.
...
**Two shapes are rejected at generation** rather than emitted:

- a dependency type in a **collection parameter** (list element, map key, map value). `list<borrow<t>>` over an imported resource is legal WIT that wit-bindgen's Rust backend cannot lower (`E0506`), so emitting it would hand you a component crate that fails to compile inside a macro expansion.
- a signature naming a dependency type the dependency's **own WIT excluded**. The error quotes the dependency's recorded reason verbatim and names the dependency, because the fix is on that side.
...
- A type marked `_CDDL_CODEGEN_EXTERN_TYPE_` in the consumer's **own** spec (a one-type re-export of a sibling crate's type) has no dependency to key on and no index to consult, so this flag cannot reach its wrappers. That case — including a sibling whose class is **hand-written**, and therefore in no generated index — is [`@extern_companions`](./comment_dsl#extern_companions).
