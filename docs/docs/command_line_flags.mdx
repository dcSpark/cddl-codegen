...
Two public functions build it. `export_schemas()` writes the document; `add_schemas(&mut schemars::SchemaGenerator)` registers this crate's types into a generator you own, and is the composition point for threading several crates' types into one document.
...
**Controlling what the document holds.** The row set defaults to the spec, and three mechanisms adjust it. Which one you want follows from *what* you are changing — a row, or a reference:
...
The two directives and the flag do not conflict: the flag consults no IR, so an extra root naming a `@no_json_schema_export` type re-registers it, and `@custom_json` + `@no_json_schema_export` on one rule is legal and means both things at once.
