...
- The union is keyed by the wrapper's **shape**, not its name; the derived structural name is cross-checked against the listed name. Hard errors (each with an actionable message): a shape referencing an element the dependency doesn't own; a name↔shape mismatch; a shape the dependency already produces under a **non-structural rule name** (remedy: rename the rule, `@name` it, or drop it); two requested shapes deriving the same structural name; a nested shape whose inner wrapper is neither requested nor own-produced; a malformed sidecar.
