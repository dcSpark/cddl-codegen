...
Requires `--json-schema-export`: the scripts compile the per-type schemas the `wasm/json-gen` crate writes, so without them there is nothing to read (the combination is rejected up front).

What the scripts do, in order:

1. `run-json2ts.js` compiles every `wasm/json-gen/schemas/*.json` to TypeScript, de-duplicates the declarations, `JSON`-suffixes every identifier so they can't collide with the wasm-bindgen types, and writes `wasm/json-gen/output/json-types.d.ts`. A schema that fails to compile **fails the run** (non-zero exit, the error printed in full) and leaves the previous output untouched.
