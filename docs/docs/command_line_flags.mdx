...
- Because the value lands inside a turbofish in a generated file, its charset is restricted to `[A-Za-z0-9_]`, `:`, `<`, `>`, `,` and space — enough for a scoped path with generic arguments (`cddl_lib::sub::Ext<u64, String>`), not enough to introduce a comment, a statement separator or a string literal. Any other character is rejected naming the offending character.
...
Two further behaviours worth stating outright:
