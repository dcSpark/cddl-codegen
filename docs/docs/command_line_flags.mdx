...
**`json_schema_gen.rs`, and how to put it behind a cargo feature.** Under `--json-schema-export` the exported set gains `json_schema_gen.rs`: the row `Registrar` (which owns the ledger carrying the published-name injectivity guard), the `add_schema` helper it delegates to, and `check_schema_ref_closure` (the document's reference-closure check). Every generated `wasm/json-gen` crate in the workspace `use`s them from here — `use <override>::json_schema_gen::Registrar;` — instead of carrying its own emitted copy, so N json-gen crates share one implementation. Like every other exported file it needs a hand `pub mod json_schema_gen;` in the target crate root the first time it appears (the notice above names it), and the merge asserts the `schemars` + `serde_json` it references.
