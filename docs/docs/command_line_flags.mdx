...
2. `json-ts-types.js` rewrites the wasm-pack `.d.ts` in place: it retypes each class's `to_json_value(): any` to that class's `<Class>JSON` type and appends the emitted declarations under a marker line. A class with no emitted JSON type keeps its `any` (a name nothing declares would be a `TS2304`), and the appended block is truncated and rewritten on every run, so re-running without a `rimraf ./pkg` is a fixed point rather than a duplicate-declaration error.
