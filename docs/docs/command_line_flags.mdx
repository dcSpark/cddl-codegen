...
**The tool does not touch `wasm/json-gen/Cargo.toml` for this.** It knows the crate name but not its path or version, and that dependency is yours to declare — exactly as `--json-schema-root`'s cross-crate roots are. The manifest is **merged**, never clobbered, so the `[dependencies]` entry you add there by hand survives regeneration. A name the manifest does not depend on is an `E0433` in your own json-gen build naming the crate, never a generation-time reject.
