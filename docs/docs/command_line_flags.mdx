...
crate in one run. `--config` is mutually exclusive with the flags below.
...
**The tool does not touch `wasm/json-gen/Cargo.toml` for this.** It knows the crate name but not its path or version, and that dependency is yours to declare — exactly as `--json-schema-root`'s cross-crate roots are. The manifest is **merged**, never clobbered, so the `[dependencies]` entry you add there by hand survives regeneration. The same hand-manifest contract covers one more entry under `--common-import-override`: the json-gen crate imports `Registrar` / `check_schema_ref_closure` from `<override>::json_schema_gen`, and that crate is a Rust path prefix the tool cannot turn into a cargo package name (`crate::common` is a legal value), so the dependency on the common crate is yours to declare there too — again surviving regeneration through the merge. A name the manifest does not depend on is an `E0433` in your own json-gen build naming the crate, never a generation-time reject.
