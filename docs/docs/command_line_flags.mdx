...
so the generated crate fails to build with your original comment in the message; you re-place or discard it and delete the block. The block carries forward verbatim on every regeneration until you delete it, so the trapped comment is never lost. Outside these tagged blocks the overlay contributes only comment bytes to the output — never a code token — and running the tool twice equals running it once.

**Doc comments are tool-owned where the tool documents.** `///`/`//!` text on generated items flows from your CDDL and the `@doc` comment DSL — edit it there, not in the output. A hand edit to a tool-emitted doc block is dropped on regeneration, and a doc block whose item vanished (e.g. you deleted a documented type from the spec) drops too rather than becoming a compile error — the one deliberate silent-drop class: stale tool docs and user-edited tool docs are indistinguishable. A doc comment you add to a position the tool does **not** document is preserved like any other comment.

**v1 scope:** own-line comments only (`//`, `///`/`//!`, and `/* */` blocks that occupy their own lines). A user-added **trailing** (end-of-line) comment fails loudly with a hint to move it to its own line, after which it is preserved. If the existing file cannot be lexed (e.g. a hand edit left an unterminated string) or read, regeneration is a hard error naming the file rather than a silent clobber. A file that *stops being generated* (removed/renamed type or scope) is orphaned on disk with a stderr warning — comments there are not carried anywhere.
