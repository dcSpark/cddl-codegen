...
* Fixed values - `null`, `nil`, `true`, `false`, integers, floats, and text literals. A named
  ...
  `Option<ThatSingleton>`; `null / null` normalizes to the one null singleton rather than exposing
  two equivalent Rust states. `undefined` and byte-string literals remain unsupported fixed kinds.
