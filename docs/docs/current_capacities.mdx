...
* Type-enforced homogeneous collections - bare `[* T]` maps to `Vec<T>` and `[+ T]` / `1* T` to `NonEmptyVec<T>`; every other supported **ordinary or `@duplicates preserve`** array occurrence window (`[2*5 T]`, `[*5 T]`, `[2* T]`, `[? T]`, `[2*2 T]`) maps to `BoundedVec<T, MIN, MAX>`. A unique-key table maps `{ * K => V }` to a loose map, `{ + K => V }` to `NonEmptyMap<K, V>`, and every other window (including omitted exact-once) to `BoundedMap<K, V, MIN, MAX>`. The bounded carriers route loose API, CBOR, JSON, wasm, and component input through their `TryFrom` range-check door and expose no mutable carrier. Bounded `@duplicates preserve` tables are refused because no bounded pair-map carrier exists. See [Output format](output_format#non-empty-containers).
