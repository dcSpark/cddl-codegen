...
A **float or negative-integer literal arm** in a type choice currently fails at generation: the enum variant's name is minted from the literal's spelling (`F1.5`, `F-1`), which is not a valid Rust identifier, so generation aborts loudly (exit 1, nothing written) instead of producing a crate — for the mixed spelling (`t = 1.5 / tstr`) and the all-fixed c-style form (`t = 1.5 / 2.5`) alike, while uint, text, bool, and null literal arms are fine. The workaround is to **name the arm**: `t = 1.5 ; @name half_and_half` on the arm's line gives the variant your identifier and the choice generates. A proper identifier scheme (or a graceful refusal naming the arm) is tracked.
