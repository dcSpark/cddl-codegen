...
overlap what can follow it** — a later
declared field reachable through any run of optionals, or the open rest tail when it is reachable:
`foo = [? f0: uint, f1: uint]`. The decoder decides presence by peeking the next element's major
type, so a type-disjoint non-final optional (`foo = [? f0: uint, f1: tstr]`) decodes fine; on
overlap the peek cannot decide and the generator refuses the decoder rather than emitting one that
guesses. Remedy: make the types distinct, drop the optional, or restructure (an optional-**last**
field never needs the peek disambiguated against a follower).
...
| `memberkey.type1` | occurrence-target (6 shapes) | `m = { 2*3 tstr => uint }` |
