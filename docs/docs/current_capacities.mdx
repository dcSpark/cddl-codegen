...
* Integer width via `.size` - `u = uint .size 1` maps to `u8`, `i = int .size 8` to `i64`, etc. Note: RFC 8610 defines `.size` only for strings and `uint`; the signed-`int` form is a cddl-codegen interpretation (the signed two's-complement window) that reference validators do not share — the ruby tool applies the unsigned window (rejecting every negative) and the rust tool refuses to evaluate it.
