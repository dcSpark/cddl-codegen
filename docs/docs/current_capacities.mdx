...
* Integer width via `.size` - `u = uint .size 1` maps to `u8`, `i = int .size 8` to `i64`, etc. Note: the `i64` reading of `int .size 8` deviates from the spec semantics as clarified by the RFC author ([cbor-wg/cddl#32](https://github.com/cbor-wg/cddl/issues/32)): `int = uint / nint` and a control distributes over the choice, so `int .size N` matches exactly the `uint .size N` window `0...(256**N)` — negatives never match (`.size` is not defined on `nint`, and an undefined control application is a non-match). cddl-codegen's `i{8N}` mapping therefore accepts negatives the spec excludes and rejects `[2^(8N-1), 2^8N)` values the spec admits. If you mean an N-byte signed integer, the conformant spelling is the explicit range (e.g. `-9223372036854775808..9223372036854775807`), which is fully supported.
