...
A **float or negative-integer literal arm** whose variant name would be **derived** is **rejected gracefully at parse**, naming the rule, the arm and the remedy. Such an arm has no member key to name its variant after, so the name is minted from the literal's spelling (`1.5` → `F1.5`, `-1` → `U-1`) — not a valid Rust identifier. The rejection covers every arm-naming path onto that minter: the mixed type choice (`t = 1.5 / tstr`), the all-fixed c-style form (`t = 1.5 / 2.5`), a nested anonymous choice (`x = [1.5 / tstr]`), and the **bare-member group-choice** spelling (`t = [ true // 1.5 ]`). Uint, text, bool and null literal arms mint identifiers and are unaffected, as is the **named-member** group-choice spelling (`t = [ true // v: 1.5 ]`), whose variant is named after the key. The supported route is to **name the arm**: `t = 1.5 ; @name half_and_half` on the arm's line gives the variant your identifier and the choice generates. Auto-sanitizing the derived name (`F1_5`) is deliberately not done — a variant name is public API of the generated crate, so the author picks it.
