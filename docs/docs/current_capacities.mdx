...
* CDDL Generics - `foo<T> = [T]`, `bar = foo<uint>`, including an instantiation as a homogeneous array element (`bars = [* foo<uint>]` — the anonymous instance is registered and emitted). A *bare* instantiation in member position (`a = [foo<uint>, tstr]`) is not yet supported.
