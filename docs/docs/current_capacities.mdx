...
* Open struct-maps (a struct-map mixing fixed members with a trailing `* K => V` rest row) - `foo = { 1: uint, * uint => any }`: unknown map entries are **captured** into a generated `rest` field instead of being an error. Every rest-row occurrence is supported and belongs to that row alone: loose `*`/`0*`, `NonEmptyMap`/`NonEmptyPairMap` for `+`/`1*`, or a checked `BoundedMap`/`BoundedPairMap` for every other window. Restricted rows are checked constructor arguments; CBOR/JSON stage loose data then enter one `TryFrom` door, while wasm/component re-enter the same carrier. The key domain is general (`uint`/`text`/`any` plus bytes, nint, sized ints, bool, unions, structs, tagged/`.cbor` domains, externs, generic instances and aliases; float-containing and `null`-admitting keys reject). `@name`/`@duplicates` apply; `@ignore` remains loose-only because dropping a restricted row would re-serialize an invalid count. See [Open struct-maps](output_format#open-struct-maps-rest-rows).
* Open arrays (an array mixing fixed members with a trailing `* t` or `+ t` rest tail) - `foo = [uint, tstr, * uint]` captures a loose generated `rest: Vec<T>` that defaults empty, while `foo = [uint, + uint]` / `1* uint` captures `rest: NonEmptyVec<T>` and takes its first tail element in `new()`. Both tails must be final and follow ≥1 fixed member. The restricted flavor routes CBOR and JSON staging through its single `TryFrom<Vec<T>>` door, so zero tails reject; JSON is required with `minItems: 1`, and wasm/component use the same first-element construction boundary plus a restricted-list getter. `@name` renames the tail field; preserve/canonical round-trip each element byte-exact in position order. The loose flavor alone keeps skip-if-empty/default-on-read JSON and may use [`@ignore`](comment_dsl#ignore); ignoring a required tail is rejected because it would re-emit zero occurrences. See [Open arrays](output_format#open-arrays-rest-tails).
