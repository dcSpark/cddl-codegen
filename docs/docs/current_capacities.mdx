...
* Open arrays (an array mixing fixed members with a final occurrence-bearing rest tail) - `foo = [uint, tstr, * uint]` captures a loose generated `rest: Vec<T>` that defaults empty; `foo = [uint, + uint]` / `1* uint` captures `rest: NonEmptyVec<T>` and keeps its first-tail-element `new()` ABI; every other final window such as `2*3`, `*3`, `2*`, or `*0` captures `rest: BoundedVec<T, MIN, MAX>` and takes that complete checked carrier in `new()`. Tails must be final and follow ≥1 fixed member. Restricted CBOR and JSON staging enter one `TryFrom<Vec<T>>` door; JSON is required with its actual `minItems`/`maxItems`, wasm uses the matching checked list wrapper, and component re-enters the same carrier from `list<T>`. `@name` renames the tail field; preserve/canonical round-trip each element byte-exact in position order. The loose flavor alone keeps skip-if-empty/default-on-read JSON and may use [`@ignore`](comment_dsl#ignore); dropping a restricted tail either re-serializes zero against a positive minimum or loses a zero-minimum window's bounded/exact state, so it is rejected. See [Open arrays](output_format#open-arrays-rest-tails).
