...
* Open arrays (an array mixing fixed members with occurrence-bearing segments) - a final tail such as `foo = [uint, tstr, * uint]`, and a sole safe leading/middle segment such as `foo = [uint, * bytes, tstr]` or `foo = [* fixed_choice, 2]` where `fixed_choice = 0 / 1`, are supported. Multiple segments are also supported when every segment is a finite exact `N*N` window other than `1*1` and has a unique explicit `@name`: each becomes its own flat `BoundedVec` field and count-owned wire boundary, so adjacent/same-major segments do not require peeking or suffix speculation. Loose `*`/`0*` captures `rest: Vec<T>` and defaults empty; `+`/`1*` captures `rest: NonEmptyVec<T>` and keeps its first-element `new()` ABI; every other window such as `2*3`, `*3`, `2*`, or `*0` captures `rest: BoundedVec<T, MIN, MAX>` and takes that complete checked carrier in `new()`. A sole variable middle segment always needs an immediate mandatory fixed suffix that expands to exactly one CBOR item. A variable-cardinality window additionally needs either a field-codec-free, effective, CBOR-major-disjoint boundary (generator-proven heads or a transparent custom alias's declared [`@custom_wire_major`](comment_dsl#declaring-the-wires-major-type-custom_wire_major)) or untagged generator-owned finite fixed-value domains on both sides with no CDDL value in common. The latter tries the repeated decoder on the real cursor, retaining its value and preserve sidecars only on success; a failed attempt restores the cursor for the suffix. Exact `2*2` stops by count and needs neither proof. Optional-prefix lookahead stays generator-proven-only, so a custom codec or opaque extern there is serialize-only unless mandatory outer tag/`.cbor` framing proves the distinct head. Variable multiple occurrences, general/residue same-major discrimination, direct opaque extern heads, and group/group-choice/plain-group forms remain graceful rejections. Restricted CBOR and JSON staging enter one `TryFrom<Vec<T>>` door; JSON is required with its actual `minItems`/`maxItems`, wasm uses the matching checked list wrapper, and component re-enters the same carrier from `list<T>`. `@name` names each multi-segment field; preserve/canonical round-trip each element byte-exact in source position. The loose flavor alone keeps skip-if-empty/default-on-read JSON and may use [`@ignore`](comment_dsl#ignore); dropping a restricted segment either re-serializes zero against a positive minimum or loses a zero-minimum window's bounded/exact state, so it is rejected. See [Open arrays](output_format#open-arrays-rest-tails).
