...
phase. **Type-choice arms**: `any` as a type-choice alternative (`a = any / tstr`, `a = [* any] / tstr`)
is not yet supported and is rejected gracefully at generation — a last-position `any` arm needs forced
backtracking semantics that also arrive in the later phase. Note the distinct construct `#`
