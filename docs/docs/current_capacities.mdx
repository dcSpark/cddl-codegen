...
**Duplicates policy.** By default a collapsed set is **preserve** — duplicate entries on the wire are accepted and re-emitted byte-exactly, in wire order (`Vec`/`NonEmptyVec` alias targets). This is the only correct default for a multi-era reader: historical on-chain data can carry duplicates a full node must round-trip exactly. A validating writer that wants set semantics opts into rejection per-rule with `; @duplicates reject` (see [comment DSL](comment_dsl#duplicates)): the transparent alias then targets an order-preserving, duplicate-free `OrderedSet<a>` / `NonEmptyOrderedSet<a>` whose single `TryFrom` door refuses a duplicate identically on the wire (`DeserializeFailure::DuplicateKey(Key::Uint(i))`) and through the API — and this holds across the Rust, JSON, and **wasm** boundaries, so a reject set that generates for Rust also generates for wasm (subject to the same pre-existing bytes-element wasm limitation noted for inline `[* bytes]` collections). Writing the default `; @duplicates preserve` on a set is accepted as self-documentation (a no-op).
