...
* A **map-representation record with a plain-group field** (`foo = { a: uint, bar }` with
  `bar = (c: uint, d: tstr)`): map entries can arrive in any order, which does not fit the
  embedded-group decode shape. Remedy: inline the group's fields or make `bar` a struct of its own.
