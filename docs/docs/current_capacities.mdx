...
**4. Wasm class re-key.** A generic set instance previously converged its wasm wrapper onto the structural list/`OrderedSet` class name; it now surfaces one **nominal class per instantiation** (`SetKeyHash`, `SetU64`), and an inline `#6.258([* T])` occurrence likewise surfaces its shape-derived nominal class. The structural boundary class (`KeyHashList`, `U64OrderedSet`) is **not** eliminated — it is the nominal's own `new()`/`get()` element-crossing boundary and stays own-produced, so a `--wrapper-requests` consumer importing the structural name is still satisfied; a named binding of an instance stays a passthrough (`pub type Named = SetKeyHash;`). Downstream JS / `--wrapper-requests` code that imported a generic set instance under its *structural* name should move to the nominal class. See [Wasm differences](wasm_differences#heterogeneous-arrays).
...
Depth is the only hostile-input class that needs an opt-in flag. The other classes are handled by the runtime (`cbor_event` 3.x): truncated input, over-claiming length headers, reserved simple values, and stray break codes all return a `DeserializeError` — no panic, and no allocation sized from an untrusted claimed length (an 8-byte header claiming gigabytes errors before any allocation).
