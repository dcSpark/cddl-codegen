...
**Known defect — map-representation group choices.** For a *map*-shaped group choice whose arm collapses to a single field (`foo = { a: uint // b: tstr }`), the generated serializer currently drops the member key and emits a truncated CBOR item (a 1-entry map header followed by only the value); the generated deserializer mirrors the same wrong shape, so it round-trips against itself but no other CBOR implementation can read the output, and spec-valid input is rejected. Do not use map-representation group choices until this is fixed (tracked in `cddl-matrix/ROADMAP.md` § findings, with regression pins in `tests/decode_conformance/catalog.toml`). Array-shaped group choices are unaffected.
## Type choices
