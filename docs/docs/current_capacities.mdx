...
|-----------|-------------------|---------|
| `grpent.inline_group` | occurrence-target | `a = [* (int, tstr)]` |
| `type2.array` | array-element, cbor-payload, choice-member, map-key, map-value, occurrence-target | `a = [[int]]` |
| `type2.map` | array-element, cbor-payload, choice-member, generic-arg, map-value, occurrence-target | `a = [{x: int, y: uint}]` |
