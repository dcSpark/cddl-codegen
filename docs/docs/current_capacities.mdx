...
refuses the decoder rather than emitting one that guesses. Remedy: make the types distinct, drop
the optional, or restructure (an optional-**last** field never needs the peek disambiguated against
a follower).
