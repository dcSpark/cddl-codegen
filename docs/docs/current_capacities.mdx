...
* Fixed values - `null`, `nil`, `undefined`, `true`, `false`, integers, floats, and text literals. A named
  ...
  `undefined / null` is `Option<FixedUndefined>`. Byte-string literals remain unsupported.
* Optional fixed-value members - `foo = [x: uint, ? v: true]`, `{? k: 5, x: uint}` — the one-bit
  ...
  text, float, and undefined (`[x: uint, ? u: undefined, label: text]`) fixed values in both array and map
  representations
