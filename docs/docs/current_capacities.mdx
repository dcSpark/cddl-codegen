...
* CDDL Generics - `foo<T> = [T]`, `bar = foo<uint>`, in every instantiation position: rule RHS, keyed member (`x: foo<uint>`), homogeneous array element (`bars = [* foo<uint>]`), and bare member position (`a = [foo<uint>, tstr]`). In each case the anonymous instance is registered and emitted (generics on plain groups, e.g. `set<a> = (* a)`, remain unsupported). Generic defs whose body is a **collection** — `xs<a> = [* a]`, `[+ a]`, `{* k => a}`, tagged or not, including the transparent tag-set idiom described below — are also supported: each instance becomes a transparent `Vec`/`NonEmptyVec`/map alias whose use-site fields serialize identically to the non-generic equivalent.
...
> **Named non-generic 258 set rules are NOMINAL.** A named, non-generic tag-258 set rule (the two-arm
...
> **Generic** set defs/instances and **inline** occurrences remain transparent (described below); only
> named non-generic rules nominalize.

A two-arm type choice whose arms are the **same collection differing only by one tag** — `my_set = #6.258([* a]) / [* a]`, the Cardano ledger's `set`/`nonempty_set`/`nonempty_oset` shape — is **not** two types: both arms denote the same logical value, and which arm was written is an encoding detail. cddl-codegen recognizes this structurally (any tag number, not just the IANA finite-set tag 258) and **collapses it into one transparent collection** — the same registration a bare `#6.N([* a])` array rule generates (its alias target — `Vec`/`NonEmptyVec` or the `OrderedSet`/`NonEmptyOrderedSet` uniqueness twin — depends on the effective duplicates policy; see **Duplicates policy** below) — instead of a `MySetArrOrArr` enum that would leak the encoding into the type. The `[+]` flavor collapses to `NonEmptyVec` (distinct from the `[*]`/`Vec` flavor) and enforces its "at least one" bound through the single `TryFrom` door. Recognition is **unconditional and structural** (no comment directive): the arm distinction carries no type-level information, so the collapse is the correct default and the enum was the accident. It works for named rules and for CDDL generics (`set<a> = #6.258([* a]) / [* a]` with `set<uint>` instances): a generic instance used as a record field (`holder = [items: set<uint>]`) serializes through the transparent alias identically to the non-generic equivalent — the generic and non-generic paths converge on one resolution, so both wire arms roundtrip byte-exact under `--preserve-encodings`. On the **wasm** boundary an **anonymous** instance at a field site (`[pool_owners: set<key_hash>]`) lowers exactly like the inline collection it denotes. When the element needs a wrapper class it lowers to the **structural** wrapper name — `KeyHashList` / `NonEmptyKeyHashList`, exactly the inline `[* key_hash]` / `[+ key_hash]` equivalent — rather than minting a class named after the synthesized instance (`SetKeyHash`); the synthesized name survives as a transparent wasm `pub type` alias to it. When the element is directly wasm-exposable (`set<uint>`) it lowers to a bare by-value `Vec<u64>` with **no** wrapper class at all, exactly the inline `[* uint]` — its wasm output is byte-identical to the inline equivalent. Either way two spellings of one shape are one JS surface, and a downstream `--wrapper-requests` consumer importing the structural name is satisfied by the dep's own spec. A **named** instance rule (`named_set = set<key_hash>`) instead keeps its own class under the rule name (the author named it deliberately, so the criterion-8 wrapper-requests contract still applies to it).
...
**Inline positions get the reject default too.** The registry is not limited to named rules: an **inline** `#6.258([* a])` / `#6.258([+ a])` written directly at a member, optional member, array-element, map key/value, or generic-argument position also carries set semantics and **defaults to `@duplicates reject`** ⇒ `OrderedSet<a>` / `NonEmptyOrderedSet<a>`, exactly as the equivalent named rule would. The same shape guard applies inline — an inline record-shaped `#6.258([uint, text])`, tagged primitive `#6.258(text)`, or tagged map `#6.258({* k => v})` gets nothing. Because an inline occurrence has **no comment slot** for `@duplicates`, the opt-out is to **hoist** it to a named rule that carries the directive (`input_set = #6.258([* input]) ; @duplicates preserve`, then reference `input_set`); a generation-time notice prints this recipe at every inline 258 site. One boundary: the two-arm *collapse* (`#6.258([* a]) / [* a]` → one transparent collection) remains **named-rule-only** — an inline two-arm choice is not collapsed (it stays a two-variant enum, per **Near misses** below), so only its tagged arm acquires the `OrderedSet` twin while the untagged arm stays a plain `Vec`. Opting an inline occurrence out is therefore always the hoist, never an inline directive. A second
boundary: an inline `#6.258` array **nested inside a named two-arm idiom rule**
(`foo = #6.258([* #6.258([* uint])]) / [* #6.258([* uint])]`) does **not** receive the inline
default — the outer rule collapses and takes the rule-level default, but the nested occurrence
stays `Vec` (pinned by `nested_inline_258_inside_named_idiom_keeps_vec_documented_boundary`); the
remedy, as always inline, is hoisting the nested occurrence to a named rule.
