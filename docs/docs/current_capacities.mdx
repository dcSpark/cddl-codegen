...
* **Worse as a member.** A byte-string literal is listed above as rejected gracefully, and that
  holds for `x = h'0102'` (exit 1, with a message). In member position — `[v: h'0102', x: uint]`,
  the unkeyed `[h'0102', x: uint]`, and `{k: h'0102', j: uint}` — the generator instead **aborts**
  (exit 101). That is a defect, not a design boundary; it is tracked in `cddl-matrix/ROADMAP.md`
  under "A byte-string literal as a fixed MEMBER value crashes the generator".

`undefined` is the one constant with no such split: it aborts in every position, which is what its
row already reports.
