...
* Floats - **the CDDL name decides which CBOR heads a member accepts, and which one it writes.** RFC
  8610's prelude defines the six names as representation types (`float16 = #7.25`,
  `float32 = #7.26`, `float64 = #7.27`, plus the two unions and the catch-all `float`), and the
  generated codec honors that literally, in both directions and in every profile:

  | CDDL name | accepts (decode) | Rust type | writes a fresh value, default profile | …under preserve / canonical |
  |---|---|---|---|---|
  | `float16` | `0xf9` | `f32` | `0xf9` | `0xf9` |
  | `float32` | `0xfa` | `f32` | `0xfa` | `0xfa` |
  | `float64` | `0xfb` | `f64` | `0xfb` | `0xfb` |
  | `float16-32` | `0xf9`, `0xfa` | `f32` | narrowest of the two that is exact | same |
  | `float32-64` | `0xfa`, `0xfb` | `f64` | narrowest of the two that is exact | same |
  | `float` | `0xf9`, `0xfa`, `0xfb` | `f64` | `0xfb` | narrowest of the three that is exact |

  A head outside the declared set is a **decode error** with the usual location annotation — never a
  silent narrowing or widening of the value, so a member's decoded value never depends on bytes its
  own type forbids. Under `--preserve-encodings` the head width is ordinary encoding data: a
  *recorded* head replays byte-exactly (it is in-set by construction), and only a member with no
  record falls back to the write column above. `--canonical-form` leaves a declared-width name alone
  — its declared width *is* its canonical form. The narrowest-lossless rule is therefore where RFC
  8949 § 4.1 preferred serialization actually has a choice to make: always for the two union names,
  and for `float` in every profile except the default one, which writes `0xfb` unchanged.

  Because every admitted head is exact for its Rust type, decoding never loses precision and a
  **NaN payload survives byte-exactly** at every width (both conversions are done in software, so no
  platform-specific float cast can quiet a signaling NaN or drop its payload).

  The one place the strictness is visible on the *write* side is `float16`, whose single admitted
  head cannot represent every `f32`. Serializing a `float16` member holding, say, `1.1` **fails**
  rather than rounding — rounding to fit is exactly the silent value mutation a declared width
  exists to prevent. The error is returned through `Serialize`, but note that the convenience
  `to_cbor_bytes()` door unwraps it, so that door **panics** on such a value; call
  `cbor_event::se::Serialize::serialize` directly (or keep the value f16-exact) where the input is
  untrusted. This is the pre-existing design of that door, not specific to floats
* Array values - `[uint]`
