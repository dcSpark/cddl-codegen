...
* Type-enforced homogeneous collections - bare `[* T]` maps to `Vec<T>` and `[+ T]` / `1* T` to `NonEmptyVec<T>`; every other supported array occurrence window maps to `BoundedVec<T, MIN, MAX>`, except `@duplicates reject`, which maps to `BoundedOrderedSet<T, MIN, MAX>` and enforces both order-preserving uniqueness and bounds at one door. A unique-key table maps `{ * K => V }` to a loose map, `{ + K => V }` to `NonEmptyMap<K, V>`, and every other window (including omitted exact-once) to `BoundedMap<K, V, MIN, MAX>`. The bounded carriers route loose API, CBOR, JSON, wasm, and component input through their `TryFrom` door and expose no mutable carrier. Bounded `@duplicates preserve` tables are refused because no bounded pair-map carrier exists. See [Output format](output_format#non-empty-containers).
...
A literal-key arrow spelling `k => v` is equivalent to the colon spelling `k: v` (the same wire entry per RFC 8610) — single- and multi-entry alike — and generates identically. A *non-literal* arrow with no occurrence marker — `{ tstr => uint }` — has RFC 8610's exact-once window and becomes `BoundedMap<K, V, 1, 1>`, never a loose map. `{ ? tstr => uint }`, `{ 2*3 tstr => uint }`, `{ *3 tstr => uint }`, `{ 2* tstr => uint }`, and `{ 0*3 tstr => uint }` likewise become their corresponding inclusive `BoundedMap` window (`u64::MAX` represents an absent upper bound). `+` / `1*` retains `NonEmptyMap<K, V>` and `*` remains loose. The checked `TryFrom` door is shared by API, CBOR, JSON, wasm, and component construction. Bounded `@duplicates preserve` tables are refused rather than silently routed through a unique-key map.
