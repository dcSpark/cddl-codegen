...
* Type-enforced homogeneous arrays - bare `[* T]` maps to `Vec<T>` and `[+ T]` / `1* T` to `NonEmptyVec<T>`; every other supported **ordinary or `@duplicates preserve`** array occurrence window (`[2*5 T]`, `[*5 T]`, `[2* T]`, `[? T]`, `[2*2 T]`) maps to `BoundedVec<T, MIN, MAX>`. Each routes loose API, CBOR, and JSON input through the same `TryFrom<Vec<T>>` range-check door; mutable element access is safe but no mutable `Vec` is exposed. A bounded `@duplicates reject` array remains an `OrderedSet<T>`/`NonEmptyOrderedSet<T>` with its existing runtime cardinality check; no `BoundedOrderedSet` is implemented. The wasm surface uses restricted Min/Max list classes for the `BoundedVec` forms and schemars publishes exact item limits. Tables retain only the non-empty `NonEmptyMap<K, V>` support; other count-permitting table markers (`?`, `n*m`, …) are rejected gracefully. See [Output format](output_format#non-empty-containers).
...
cause. The remaining array-record case is an optional field immediately reachable before an **open
rest tail** with an overlapping CBOR major type, for example `foo = [? f0: uint, * uint]`: neither
a peek nor a definite header can say whether the next uint belongs to `f0` or the tail. Remedy:
make the types distinct, drop the optional or tail, or restructure.

An ordinary non-final optional followed by a mandatory field is supported even when their CBOR
major types overlap: `foo = [? f0: uint, f1: uint]`. On a **definite** array, the decoder reserves
the entire mandatory suffix and uses the remaining element count before its existing major-type
peek, so `[uint]` reads `f0` as absent and `[uint, uint]` reads it as present. The generated
default and canonical serializers always write this actual definite count. An **indefinite**
encoding of that overlapping shape is rejected at runtime with `Cannot decode an
indefinite-length array with an optional field overlapping a later mandatory field`; preserve mode
therefore never re-mints that ambiguous form.

The open-tail refusal **propagates to every container of such a type**: an enum with it in any arm (both
