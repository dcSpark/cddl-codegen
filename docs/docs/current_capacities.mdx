...
non-choice position: a struct-map member value (`{ 1: any }`), an array-record member (`[any, uint]`),
a homogeneous array element (`[* any]`), a table domain and/or range (`{ * any => any }`,
`{ * uint => any }`, `{ * any => uint }`), a top-level alias (`x = any`), and a tagged rule
(`t = #6.11(any)`). It lowers to the `AnyCbor` runtime value — see
[The `any` type](output_format#the-any-type-anycbor-runtime-value) for the runtime type, its
...
Two limits apply today. **Surfaces**: only the Rust surface is generated. A spec whose types contain
`any` is rejected gracefully at generation time under `--wasm=true` or the JSON flags
(`--json-serde-derives` / `--json-schema-export`), each with a message naming the workaround
(`--wasm=false`, or drop the JSON flags); the wasm wrapper and JSON/schema surfaces arrive in a later
phase. **Type-choice arms**: a bare `any` arm (`a = any / tstr`) is not yet supported and is rejected
gracefully at generation — a last-position catch-all `any` arm needs forced-backtracking semantics that
also arrive in the later phase. A *container* of `any` as an arm (`a = [* any] / tstr`) is an ordinary
supported arm today: `[* any]` is a homogeneous array whose CBOR wire type (Array) is distinct from the
other arms', so the existing choice machinery dispatches it — only the bare arm, which overlaps every
other arm, waits on the catch-all semantics. Note the distinct construct `#`
