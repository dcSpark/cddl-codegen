...
* CDDL Generics - `foo<T> = [T]`, `bar = foo<uint>`, in every instantiation position: rule RHS, keyed member (`x: foo<uint>`), homogeneous array element (`bars = [* foo<uint>]`), and bare member position (`a = [foo<uint>, tstr]`). In each case the anonymous instance is registered and emitted (generics on plain groups, e.g. `set<a> = (* a)`, remain unsupported).
