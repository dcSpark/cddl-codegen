...
```rust
#[derive(Clone, Debug)]
pub struct SpecialHash(pub(crate) Vec<u8>);
...
Note that signed types use explicit ranges, not `.size`: `int .size N` is rejected, because its
spec meaning is the **unsigned** window (`.size` is not defined on negative integers —
[cbor-wg/cddl#32](https://github.com/cbor-wg/cddl/issues/32)), which is spelled `uint .size N`.
