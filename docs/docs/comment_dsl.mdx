...
Two honest limits of that routing, both currently pinned rather than fixed:

* Writing the directive on the inline arm's own row (`{ * k => v ; @duplicates preserve` on the
  line, then the closing brace) is **accepted and silently ignored** today — the loose container is
  emitted and duplicate-keyed bytes fail `DuplicateKey` with no hint the directive was dropped.
  Treat the silence as unspecified, not as a stance; it is pinned as a known silent drop and will
  become either honored or a loud rejection.
* For a **self-referential** union (`md = { * md => md } / …`, the transaction-metadatum shape),
  the named-rule spelling currently **aborts generation** (the union-rooted registration-ordering
  panic pinned by `tests/robustness/recursive_union_keyed_table_nominal.cddl`) — so a recursive
  metadatum map cannot carry `@duplicates preserve` in either spelling yet. Non-recursive unions
  route through the named rule exactly as the example above shows.
...
* **On a rule that mints a TAGGED wrapper without the directive** — a tag-head rule (`foo = #6.42(uint)`) and the tag-258 set idiom (`foo = #6.258([* uint]) / [* uint]`, which nominalizes into a set wrapper). Structurally the `@newtype` case above, minus the directive: the same generated-`Serialize`/custom-reader asymmetry, so either half is refused. Declare the rule `_CDDL_CODEGEN_EXTERN_TYPE_` and hand-write the type, or give it a body that resolves to a transparent alias and write the wire framing (the tag included) in your own codec.
...
Known limitation: a tag-head rule **wrapping** an annotated alias does not route through the pair —
...
`foo`'s generated codec calls neither function on that path, which is why the tagged-wrapper rejection above does not advertise this spelling as a remedy. Until this is fixed, hand-own the wrapping rule (`_CDDL_CODEGEN_EXTERN_TYPE_`), or give the wrapping rule a transparent-alias body and put the tag framing inside the pair itself.
