...
not a convenience. On any other arm the directive is read as that variant's own metadata, where
`@custom_json` means nothing, and it is silently dropped. So reordering a type choice's arms can
turn the directive off: if you move the arm that carries it out of last place, the type gets its
JSON derives back, and they will collide with the impls you wrote by hand. Keep the directive on
whichever arm is last.
