...
becomes an order-preserving, duplicate-free set on every target. The no-op defaults (`preserve` on a
set, `reject` on a table) are accepted for self-documentation. The one stance not yet built is
**`preserve` on a table** (the byte-exact duplicate-keyed pair-map representation); using it aborts
generation with a clear message pointing at the tracked phase-2 work.
:::
