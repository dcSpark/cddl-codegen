...
types that door covers: written on a member whose type is anything else — a homogeneous `[* uint]`,
a primitive, a named rule, an array behind a tag, an inline map — the name is read by nothing, and
it is a hard error naming the arm's own slot (`// ; @name <n>`) as the remedy. The arm's own name
always comes from the arm slot; the entry slot never names the variant.
