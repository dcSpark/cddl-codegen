...
- **Rejected combinations**, each a graceful generation error naming the remedy:
  - **A restricted map rest row** (`+`, `1*`, or another bounded window): dropping its captured
    entries would always re-serialize zero occurrences and violate the declared row cardinality.
    Use capture so the checked `NonEmptyMap`/`BoundedMap` carrier retains the entries. (Restricted
    heterogeneous array tails remain outside the supported array-rest shape except for `+`/`1*`,
    which reject `@ignore` for the same reason.)
