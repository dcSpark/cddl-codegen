...
That routing works for a **self-referential** union too (`md = md_map / [* md] / …` with
`md_map = { * md => md } ; @duplicates preserve`, the transaction-metadatum shape) — in either rule
order, whether the cycle is written starting from the union or from the map. One honest limit of the
routing is currently pinned rather than fixed:
