...
The pair parses in the positions below but cannot be honored there, so each one is a graceful generation error naming the spelling that works. The common cause is that the pair is a **type-level** override: it replaces the codec of the rust type the rule resolves to, keyed on that type's alias node. Each of these positions deletes, bypasses, or never had that node.

* **On a `_CDDL_CODEGEN_EXTERN_TYPE_` rule.** An extern rule names a type this crate does not define, so that type owns its own (de)serialization impls. Give the rule a real CDDL body and put the pair there — that also keeps the wire type stated in the spec.
...
Two positions remain accepted-and-unhonored. Honoring or refusing either needs a design decision rather than a call-site fix, so treat the current silence as unspecified rather than as a guarantee.
