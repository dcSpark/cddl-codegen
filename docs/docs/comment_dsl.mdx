...
Its JSON face is [one flattened object holding both rows](output_format#open-tables-a-typed-row-plus-a-catch-all), read back typed-first — worth reading before choosing a `K_t`, because a key type admitting every member name leaves the catch-all unreachable through JSON.
