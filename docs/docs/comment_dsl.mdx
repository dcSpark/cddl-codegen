...
Using it anywhere other than a locally-scoped **non-generic** `_CDDL_CODEGEN_EXTERN_TYPE_` rule is a
hard error: on a
...
for. Declare the concrete shape as its own non-generic extern rule and put the deferral there. A
