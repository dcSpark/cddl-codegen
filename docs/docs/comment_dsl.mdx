...
On a `_CDDL_CODEGEN_EXTERN_TYPE_` or `_CDDL_CODEGEN_RAW_BYTES_TYPE_` rule the directive is accepted
and does nothing: the named type is hand-written in full, so this crate emits no JSON derives for it
either way. It is a redundant no-op rather than a dropped intent, which is why it is documented here
rather than refused.
