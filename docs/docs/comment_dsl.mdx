...
### Wrapping an annotated alias

A rule that **wraps** an annotated alias — a tag head, or `@newtype` — routes through the pair in both directions, owning only the framing it declares:
...
`Foo` writes tag 42 and then calls `my_ser`; it reads tag 42 and then calls `my_deser`. `direct.d` calls the same pair without a tag. So `inner` has one wire form however it is reached, and the wrapper adds exactly the tag its own rule declares — which is why the [tagged-wrapper rejection](#positions-that-are-rejected) above names this spelling as the remedy for putting the pair on the tag rule itself.

Under `--preserve-encodings` the split follows the same line: the tag's own `Sz` stays a generated-code-owned slot on the wrapper's encoding struct, and the payload's encoding variables are the ones that cross the call — [declared](#declaring-the-wires-encoding-variables-custom_encodings) by the alias's `@custom_encodings` if it carries one, inferred from the aliased type otherwise.
