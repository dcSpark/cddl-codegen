...
the non-empty `{+ …}` flavor (its single `try_from` door composes the min-1 check). The no-op defaults
(`preserve` on a set, `reject` on a table) are accepted for self-documentation. A preserve table and a
non-preserve map/table of the *identical* key/value shape cannot coexist under `--wasm` (they would
