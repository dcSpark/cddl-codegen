...
:::warning Status
`@duplicates` is currently **recognized but not yet active**: the parser understands it and
validates its placement, but no generation behavior is wired up yet, so **using it aborts
generation** with a clear message. This status line is removed when the feature activates.
:::
