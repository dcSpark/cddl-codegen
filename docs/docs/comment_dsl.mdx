...
- It is **rule-scoped**: the tag names the type whose wrapper to mint, so it belongs on a rule. At a
  field/member position (`holder = [f: bw, ; @used_as_elem]`) it is a hard error — put it on the rule
  that defines the element type instead (`bw = [...] ; @used_as_elem`).
