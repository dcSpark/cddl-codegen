...
- **Placement.** `@ignore` is bare (no argument) and reads from the trailing entry's **own comment
  ...
  recognized **loose** rest row of a map-rep record or a loose rest tail of an array-rep record;
  anywhere else (a rule, a field, a non-final occurrence, or a restricted row/tail) is a loud
  graceful rejection, never silently dropped. The same **marker-slot trap** applies: a directive glued to the `*` occurrence marker's
...
- **Rejected combinations**, each a graceful generation error naming the remedy:
  - **A restricted map rest row or open-array tail** (`+`, `1*`, or another bounded window):
  ...
  - **`@duplicates`** on the same entry: a duplicates policy governs a capture container, which
    ...
    — a positional sequence tail has no keys for a duplicate policy to govern.)
