...
- It is rejected if the element is *directly WASM-exposable* (e.g. a transparent `uint`/`text` alias):
  ...
  mint.
- It only mints the loose `[* x]` list wrapper. `[+ x]` (NonEmpty), bounded array wrappers, and map wrappers are out of scope
...
- **`@name`** renames the captured tail field (default `rest`) exactly as it renames the map
  capture field — `* bytes ; @name extras` yields `pub extras: Vec<Vec<u8>>`; the non-empty
  spelling `+ bytes ; @name extras` yields `pub extras: NonEmptyVec<Vec<u8>>` and its generated
  constructor's `first_extras_element` argument.
