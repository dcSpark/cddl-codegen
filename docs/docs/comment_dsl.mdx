...
Declares that a type is **not part of the published JSON-schema surface**. Its only effect: under
`--json-schema-export` the json-gen crate emits no `gen_json_schema!` row for the type, so no
`schemas/<Type>.json` is written for it.
...
- remove the `serde::Serialize`/`serde::Deserialize` or `schemars::JsonSchema` **derives** on a
  generated type. A parent that embeds the type still needs `JsonSchema` on it, and the type still
  appears inside its parents' `$defs`. Removing the derives is [`@custom_json`](#custom_json), which
  is orthogonal — combining the two on one rule is legal and meaningful ("I supply the JSON impls,
