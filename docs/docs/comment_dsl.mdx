...
- remove the `serde::Serialize`/`serde::Deserialize` or `schemars::JsonSchema` **derives** on a
  ...
  appears in the crate's `$defs` (and so in the emitted TypeScript) whenever a published type
  references it — dropping the row drops the *root declaration*, not the definition. Removing the
  derives is [`@custom_json`](#custom_json), which
