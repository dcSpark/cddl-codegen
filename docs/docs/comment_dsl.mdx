...
`to_json`/`to_json_value`/`from_json`, and the `--json-schema-export` registration row — are the
...
`--json-serde-derives`, plus `schemars::JsonSchema` under `--json-schema-export`. Those three are
...
* **On a rule that binds a generic instantiation** — `foo = base<uint>`, and the named-binding-to-a-set-nominal form of the same shape. The type is built during generic resolution from the **definition's** config, so a pair written on the binding never reaches generation and both directions keep the definition's generated codec. Moving it to the definition does not work either (a generic definition names no concrete type and is refused for that reason): give the rule a CDDL body of its own, or declare it `_CDDL_CODEGEN_EXTERN_TYPE_`.
