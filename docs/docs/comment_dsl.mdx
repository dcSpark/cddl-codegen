...
* **On the row of an *inline* anonymous map** — `f: { * k => v ; @custom_serialize … }` written directly in a member position rather than as a named table rule. The named-rule form of the same slot is rejected (above); the inline form is not reached and stays silent. Name the map as its own rule to get the diagnostic, and put the pair on its key or value rule.
* **`@custom_deserialize` on an open struct-map rest row's key-domain alias.** `@custom_serialize` *is* honored there (at both the canonical key merge and the write arm), but the read side binds the key the rest row's own dispatch already consumed, so the custom reader is never called. A transforming codec in that position writes transformed keys and reads raw ones. The same alias works normally in table and field positions, so this is a property of the rest-row use site, not of the rule.
