...
payload is — so the anonymous array or map behind it is still the only thing the name can mean:
```cddl
...
occurrence-selected `Vec<T>` / `NonEmptyVec<T>` / `BoundedVec<T, MIN, MAX>` carrier (today's wire
behavior verbatim) for a rule that must accept and re-emit historical duplicate-bearing data.
...
- **`@name`** renames the generated capture field (default `rest`) — here to `extra`. It also names
  the wasm parent-mutation door (`insert_extra(key, value)` rather than `insert_rest`); the
  containing type keeps its rule name. A field whose wasm getter would collide with that generated
  method is refused under `--wasm` with an `@name` remedy rather than silently suffixing public API.
...
- **`@name`** renames the captured segment field (default `rest`) exactly as it renames the map
  ...
  constructor's `first_extras_element` argument; a bounded spelling such as
  `2*3 bytes ; @name extras` yields `pub extras: BoundedVec<Vec<u8>, 2, 3>` and takes that complete
  checked carrier in its generated constructor.
- **`@duplicates`** does **not** apply to an occurrence segment and is rejected gracefully: an array segment is
  a positional sequence carrier in every supported occurrence flavor, so there are no keys for a
  duplicate policy to govern.
...
- **Placement.** `@ignore` is bare (no argument) and reads from the occurrence entry's **own comment
  slot**, exactly like `@name`/`@duplicates`
  ([above](comment_dsl#name-and-duplicates-on-an-open-map-rest-row)). It is valid **only** on a
  recognized **loose** rest row of a map-rep record or a loose final/safe-middle segment of an
  array-rep record. A safe middle segment needs its immediate fixed suffix to be mandatory,
  single-item, field-codec-free, and CBOR-major-disjoint from the repeated element, with no
  custom- or extern-owned, otherwise-unproven boundary wire head; this is what
  permits greedy RFC 8610 decoding without a rewind. Anywhere else (a rule, a field, a same-major
  or otherwise unsafe non-final occurrence, or a restricted row/segment) is a loud graceful
  rejection, never silently dropped. The same **marker-slot trap** applies: a directive glued to the `*` occurrence marker's
...
- **Rejected combinations**, each a graceful generation error naming the remedy:
  - **A restricted map rest row or open-array segment** (`+`, `1*`, or another bounded window):
    dropping its captured content re-serializes zero occurrences. That violates a positive minimum;
    a zero-minimum restricted window instead loses the bounded or exact state its checked carrier
    retains. Use capture so the matching `NonEmpty`/`Bounded` carrier retains the entries or
    elements.
  ...
  - **`@duplicates`** on the same entry: a duplicates policy governs a capture container, which
    `@ignore` does not create. (On an **array** occurrence segment `@duplicates` is rejected for
    *both* flavors — a positional sequence has no keys for a duplicate policy to govern.)
...
- **Typing is still enforced.** `* uint => any ; @ignore` still errors on a text key (the spec said
  uint labels), and an ignored `* text` segment still errors on a non-text repeated element; full
...
Two readers consume it: an [open table](#open-tables)'s typed row, whose dispatch must know the
claimed major *before* any deserializer runs; and a variable middle [open array](output_format#open-arrays-rest-tails)
occurrence boundary, whose greedy loop must know whether the next item is another repeat or its fixed suffix.
...
`@custom_wire_major` is refused in the same three places, plus one of its own: **on a rule nothing consumes it from** — the declared major is read only where a transparent alias keys an [open table](#open-tables)'s typed row or proves a variable middle [open-array](output_format#open-arrays-rest-tails) boundary, so a declaration reaching neither states a fact about a wire no reader needs.
