...
The shape that works, and the two constraints that are easy to get wrong:

- **Keep the schema file inside the rust crate** — `src/json/custom_schemas/<Name>.json` beside the
  impl, read with `include_str!`. This is a cargo packaging constraint, not a cddl-codegen one: a
  published crate ships only files under its own directory, so an `include_str!` reaching into a
  sibling crate compiles locally and breaks at `cargo publish`.
- **Write internal references as `#/$defs/<Name>`.** The exported document always puts its
  definitions under the literal key `$defs`, and the json-gen crate's own checks read the reference
  namespace from the generator's settings rather than assuming it, so the two agree for the default
  generator the crate builds. It is still the one spot in this pattern where a hand-written string
  encodes a decision the tool otherwise reads at runtime.

```rust
impl schemars::JsonSchema for PlutusData {
    fn schema_name() -> std::borrow::Cow<'static, str> { "PlutusData".into() }
    fn json_schema(_: &mut schemars::SchemaGenerator) -> schemars::Schema {
        serde_json::from_str(include_str!("custom_schemas/PlutusData.json"))
            .expect("PlutusData.json is not a valid schema")
    }
