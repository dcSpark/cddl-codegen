...
`@ignore` on an **open struct-map rest row** selects the *tolerate-and-drop* flavor: unknown map
entries are still typed-deserialized (key **and** value, so the stream advances past nested
containers) but then **discarded** — no capture field is generated, and serialize re-emits only the
declared members.
...
(no `@ignore`) is [capture](output_format#open-struct-maps-rest-rows), which keeps the generated
`Serialize`/`Deserialize` honest (nothing dropped, byte round-trips hold).

- **Placement.** `@ignore` is bare (no argument) and reads from the **rest row's own entry-trailing
  comment slot**, exactly like `@name`/`@duplicates`
  ([above](comment_dsl#name-and-duplicates-on-an-open-map-rest-row)). It is valid **only** on a
  recognized rest row of a map-rep record; anywhere else (a rule, a field, a table/array rule) is a
  loud graceful rejection, never silently dropped. The same **marker-slot trap** applies: a directive
  glued to the `*` occurrence marker's comment slot is silently not honored, so keep it after the
  whole entry (`* K => V ; @ignore`). Note a CDDL comment runs to the end of the line, so on a
  single-line rule `; @ignore }` would swallow the closing brace — put the directive line before
  the `}` (as in the example above).
- **Deliberate lossiness.** Byte round-trips do **not** hold for wire data carrying unknown entries —
  they are dropped, and re-serialization emits declared members only. The generated type and its
  `serialize` fn carry a rustdoc breadcrumb saying so.
- **Rejected combinations**, each a graceful generation error naming the remedy:
  ...
  - **`@duplicates`** on the same row: a duplicates policy governs a capture container, which
    `@ignore` does not create.
  - **`@name`** on the same row: there is no capture field to rename.
- **Typing is still enforced.** `* uint => any ; @ignore` still errors on a text key (the spec said
  uint labels); full key-looseness is spelled `* any => any`. A declared key wins over the rest row,
  so a fixed key whose wire value mismatches its type errors rather than falling through to the drop
  arm.

The **JSON** and **wasm** surfaces of an `@ignore` type are a plain **closed struct's** (unknown JSON
keys are tolerated on read per serde's default and dropped; there is no `rest()` wasm getter, since
nothing is stored).
