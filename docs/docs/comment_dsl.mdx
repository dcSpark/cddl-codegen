...
**Known limitation:** an input file whose rules are *all* extern currently generates its scope's `mod.rs` (carrying the `pub use crate::…;` re-export glue) without declaring that module in the generated root, so the crate fails to compile with unresolved-module errors wherever another scope references those types. Until this is fixed, keep at least one non-extern rule in any file that declares externs.
