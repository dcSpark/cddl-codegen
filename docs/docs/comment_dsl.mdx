...
wrapper, a collection typedef. On a rule that registers **no** rust type (a plain transparent alias
like `foo = uint`, or a `@no_alias` alias) there is no row to suppress, so the directive would be
silently dead and generation is rejected with an error naming the rule.
