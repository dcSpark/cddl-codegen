...
### Positions that are not honored today

The pair also parses in the positions below, where it currently has no effect — or, in the last two, only half an effect. None of them is rejected, so the directive is silently inert there; use the type-level alias spelling instead.

* **On a `_CDDL_CODEGEN_EXTERN_TYPE_` rule.** An extern type owns its own (de)serialization impls, and the parsed pair never reaches generation.
* **On a table row or an open-map rest row entry** — a trailing comment on the `* k => v` row itself. Row-entry comments carry `@name`, `@duplicates` and `@ignore` only; anything else in that slot is unconsumed. Put the pair on the row's key or value rule instead.
* **Together with `@no_alias`.** `@no_alias` removes the alias node the override is keyed on, so the pair goes with it.
* **Together with `@newtype`.** The deserialize call sites do call the custom reader, but the wrapper's serialization goes through its generated impl — an asymmetric round-trip rather than a clean drop.
* **`@custom_deserialize` on an open struct-map rest row's key-domain alias.** `@custom_serialize` *is* honored there (at both the canonical key merge and the write arm), but the read side binds the key the rest row's own dispatch already consumed, so the custom reader is never called. A transforming codec in that position writes transformed keys and reads raw ones.

Whether each of these becomes supported or a graceful rejection is not yet decided — treat the current silence as unspecified rather than as a guarantee.
