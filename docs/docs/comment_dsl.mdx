...
- **`@name`** renames the captured segment field (default `rest`) exactly as it renames the map
  ...
  checked carrier in its generated constructor; an ordinary exact `2*2 bytes ; @name extras` yields
  `pub extras: [Vec<u8>; 2]` instead. Exact `@duplicates reject` collections remain their ordered-set
  carrier because an array alone cannot enforce uniqueness.
