...
delete the orphaned import. The import set is re-derived from the *final* code after your edits are
applied, so it always tracks what those edits leave behind, and this holds even when the reference
you removed lived in a different file of the module (e.g. a `serialization.rs` edit orphaning a
`mod.rs` import).
...
unrecognized comment starting with `// cddl-codegen:`. That last rule is what keeps the
...
| `It is a trailing (end-of-line) comment` | a plain trailing comment (no `cddl-codegen:` tag) can't be carried | move it to its own line |
