...
(`BTreeMap`, `OrderedHashMap`, `NonEmptyVec`, `BoundedVec`, `NonEmptyMap`, `BoundedMap`, `OrderedSet`,
`NonEmptyOrderedSet`, `BoundedOrderedSet`, `PairMap`, `NonEmptyPairMap`, `BoundedPairMap`) only when
the file — or one of its descendant modules — references them (see [Output format](./output_format)). The relationship to
