...
One entry a `deps` edge needs is still **not** derived and remains a hand edit: under `[runtime]`,
each crate's dependency on the shared runtime crate, which `--common-import-override` documents as
your line — an override is a Rust path prefix (`crate::common` is a legal value), so no package name
