...
that umbrella crate is a workspace member (see [`--lib-name`](./command_line_flags)), rather than
...
| key | rule | why |
|---|---|---|
| `preserve-encodings` | must be **identical** in every crate | the preserve + canonical runtime carries a narrow `{+ K => V}` `BTreeMap` bridge for a reduced consumer, but that does not make arbitrary flavor mixtures derivable |
...
A workspace can legitimately want one runtime for crates that do not all agree. A preserve +
canonical carrier includes reduced-consumer bridges for `{+ K => V}` and `any`, but mixed equality
axes and depth limits remain an explicit unsupported contract. Naming the carrier by hand accepts
that:
...
through the preserve + canonical runtime's `{+ K => V}` `BTreeMap` bridge and `any`
one-argument `AnyCbor::serialize` bridge, but automatic carrier derivation still requires identical
preserve-encodings/canonical-form values; a crate whose --deserialize-depth-limit differs has its
`any` values guarded at `chain`'s limit rather than its own.
```
...
generating `cip25` against a runtime built at `chain`'s flavor. The run states the two
reduced-consumer bridges back by name, while preserving the stronger rule: automatic derivation does
not accept a mixed `preserve-encodings`/`canonical-form` configuration, and a depth-limit mismatch
still guards `cip25` values at `chain`'s limit. It is a claim about cip25's contract, and one that a
future incompatible use can falsify; the statement makes that visible on the run that introduces it.
