...
- **`--extern-import`** is derived too. CML declares its extern dependencies by hand, in
  ...
  export instead. A project in that position is choosing between the two sources, and it is a choice
  rather than a merge: a dependency named by `deps` — or by a hand-written `extern-import` entry —
  that the consumer *also* declares as a stub directory is a **hard error**, naming the key that
  declared the edge and the stub directory it collides with, during expansion and so before any crate
  generates. If you want to keep the hand-declared one, write `extern-wasm-crate`,
  `extern-wrapper-index` and `workspace-dep` as plain per-crate keys and do not use `deps`.
