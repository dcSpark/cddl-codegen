...
`key-requests`, `json-schema-dep`, `json-gen-dep` and `wasm-dep`:
...
- **`wasm = false` on the dependency** derives `--extern-import`, plus the one `--wasm-dep` naming its
  ...
  `--wasm-dep` at all, having no manifest for one to land in.)
...
A consequence worth planning for: the first run of a brand-new workspace exits nonzero, because a
cold tree genuinely does not build until the dependency has heard what its consumers borrow. The
command the verdict prints settles it, and so does a second full run.

Both checks read generated files and neither changes one. Nothing about what is generated depends on
either; the verdict's only effect is the exit code.
...
sidecar to read says so on stderr and treats the consumer as borrowing nothing, and the first run
[exits nonzero](#the-verdict-on-the-committed-tree) because the tree it wrote does not build yet.
:::
...
Two entries a `deps` edge needs are **not** derived, and both are hand edits today: the dependency's
rust crate in the consumer's own `rust/Cargo.toml`, and — under `[runtime]` — each crate's dependency
on the shared runtime crate, which `--common-import-override` documents as your line.
...
Four right-hand sides are deliberately *not* resolved. Two of them are not paths at all:
`extern-wasm-crate` names a crate, and `json-schema-dep` names a Rust module path that lands
verbatim in generated code.

`json-gen-dep` and `wasm-dep` are the other two, and they are the ones worth stating: their
right-hand sides **are** paths, and are still passed through verbatim. It becomes a cargo *path dependency* in
`<output>/wasm/json-gen/Cargo.toml` and `<output>/wasm/Cargo.toml` respectively, and cargo resolves
such a path against the manifest holding it — so rewriting it against the config file's directory
would point it somewhere cargo never looks. Count each from the crate whose manifest it lands in,
exactly as you would writing the entry by hand. (For a dependency this config contains you do not
write either at all: the [threading derivation](#the-published-json-surface) and [the wasm
manifest](#the-wasm-manifest) compute those relative paths for you.)
...
[The wasm manifest](#the-wasm-manifest).
...
multi-era **adds** a borrow leaves chain not hosting it, and the
