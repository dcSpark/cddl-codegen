...
> consumer-query contract). The matrix is **fully scaled and gate-green**: <!-- status-header counts are generated — regenerate with: cd cddl-matrix && bun run project_status_headers.ts --write --><!-- gen:sh:readme-counts -->111 features and 94 containment cells<!-- /gen:sh:readme-counts -->
> across all axes (incl. the `CDDL_CODEGEN` vendor profile), with <!-- gen:sh:readme-annotations -->239 cddl-codegen support annotations<!-- /gen:sh:readme-annotations -->,
