 ...
 *
 * The twelve marker-delimited payloads are derived and planned by the pure roadmap compatibility
 * seam.  This file deliberately remains the only live status writer: it classifies argv before it
 * can read a Markdown target, snapshots each target once for check/write, and applies a successful
 * write plan in the historical ROADMAP -> matrix README -> tests README order.
 *
...
const plan = planLegacyStatusHeaderRun(inputs, invocation);
if (plan.exit_code === 0 && mode === "write") {
  const relativeByPath = new Map(TARGETS.map((target) => [target.path, target.relative] as const));
