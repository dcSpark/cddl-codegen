...
//
// `decodeForeign` and `componentProbe` mirror verify.ts's opt-out discipline: each leg's header
// paragraph is emitted only when that leg is on, so an opted-out run's output is byte-identical to
// a pre-leg run. The committed file is written by default-on runs; the drift gate detects which
// form it holds and compares against the matching template.
// ==================================================================================================
export function annotationsHeaderLines(decodeForeign: boolean, componentProbe = false): string[] {
  return [
    ...
    ...(componentProbe ? [
      "#",
      "# COMPONENT-EXECUTION clause (the `--component` face, a BOUNDED row selection): a selected row's",
      "#   `evidence` gains one `; component …` clause naming the STAGE it was observed at — generation,",
      "#   the minted WIT door, the wasm32-wasip2 build, or the round trip itself. The leg regenerates the",
      "#   row's catalog `spec` with `--component=true --wasm=false`, builds the component, and drives the",
      "#   catalog's vectors through it under wasmtime: every accept vector must decode AND re-encode to",
      "#   the bytes the row's own rust crate produces (a live oracle, never a committed expectation), and",
      "#   every reject vector must come back as `Err` — never a trap — with the instance still usable.",
      "#   CORROBORATION ONLY, like the wasm oracle: it never changes a verdict. Rows outside the",
      "#   selection carry no component clause; compile BREADTH is the `component_corpus_compiles` gate's.",
    ] : []),
    "",
  ];
}
