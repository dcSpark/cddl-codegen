...
export function scanRoadmapCitations(
  ...
  for (const input of sorted) {
    ...
    let cursor = 0;
    while ((cursor = text.indexOf("roadmap:", cursor)) >= 0) {
      const valueStart = cursor + "roadmap:".length;
      let end = valueStart;
      while (end < text.length && /[A-Za-z0-9._-]/u.test(text[end]!)) end += 1;
      if (end === valueStart) {
        cursor = valueStart;
        continue;
      }
      ...
      const span = { start_byte: byteOffsets[cursor]!, end_byte: byteOffsets[end]! };
      if (!validation.ok) {
...
export function extractRustTestSymbols(
  ...
  if (testsMod === undefined) {
    ...
    const declaration = declaredTestModules(rustTokens(testsMod));
