...
/**
 * The oracle's ORDER now single-sources from the payload descriptor table (Packet 3A-1 collapsed
 * the hand-duplicated switch), but through this selftest-owned traversal rather than engine's
 * walk, so a traversal defect in either side still trips the comparison.  Completeness stays
 * independently proven against `markdownByteMap` (every Uint8Array anywhere in the payload).
 */
function expectedCanonicalFieldOrder(value: SemanticPayload): readonly string[] {
  const paths: string[] = [];
  const dotted = (candidate: unknown, path: string): unknown =>
  ...
  const selectArm = (arms: readonly PayloadArm[], candidate: unknown): PayloadArm => {
    const found = arms.find((entry) =>
      entry.when.every(([path, values]) => {
        const discriminant = dotted(candidate, path);
        return typeof discriminant === "string" && values.includes(discriminant);
      })
    );
    assert(found !== undefined, "payload matches no descriptor arm in the selftest oracle");
    return found;
  };
  const walk = (obj: unknown, arm: PayloadArm, prefix: string): void => {
    for (const field of arm.fields) {
      const spec = field.value;
      const prop = spec.t === "array_table" ? spec.prop : field.name;
      const fieldValue = (obj as Record<string, unknown>)[prop];
      if (fieldValue === undefined) continue;
      if (spec.t === "markdown") paths.push(`payload.${prefix}${field.name}`);
      else if (spec.t === "table") {
        walk(fieldValue, selectArm(spec.group.arms, fieldValue), `${prefix}${field.name}.`);
      } else if (spec.t === "array_table" && spec.flatten === undefined) {
        (fieldValue as readonly unknown[]).forEach((entry, index) =>
          walk(entry, selectArm(spec.group.arms, entry), `${prefix}${spec.prop}[${index}].`)
        );
      }
    }
  };
  walk(value, selectArm(PAYLOAD_KIND_ARMS[value.kind], value), "");
