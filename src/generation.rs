...
impl GenerationScope {
    ...
    pub fn generate(&mut self, types: &IntermediateTypes) {
        ...
        for (rust_ident, rust_struct) in types.rust_structs() {
            ...
            match rust_struct.variant() {
                ...
                RustStructType::Wrapper{ wrapped, control } => {
                    match rust_struct.tag() {
                        Some(tag) => generate_wrapper_struct(self, types, rust_ident, &RustType::Tagged(tag, Box::new(wrapped.clone())), control.as_ref()),
                        None => generate_wrapper_struct(self, types, rust_ident, wrapped, control.as_ref()),
                    }
                },
            }
        }
    ...
    fn generate_serialize(&mut self, types: &IntermediateTypes, rust_type: &RustType, expr: &str, expr_is_ref: bool, var_name: &str, body: &mut dyn CodeBlock, is_end: bool, serializer_name_overload: Option<(&str, bool)>, encoding_var_is_local: bool) {
        ...
        let enc_var_prefix = if encoding_var_is_local {
            ""
        } else {
            "self."
        };
        let encoding_var = format!("{}{}_encoding", enc_var_prefix, var_name);
        match rust_type {
            RustType::Fixed(value) => match value {
                ...
                FixedValue::Uint(u) => {
                    write_using_sz(body, "write_unsigned_integer", serializer_use, &format!("{}u64", u), line_ender, &encoding_var);
                },
                // TODO: should this be Nint instead of Int? CDDL spec is Nint but cddl lib is Int
                FixedValue::Int(i) => {
                    write_using_sz(body, "write_negative_integer", serializer_use, &i.to_string(), line_ender, &encoding_var);
                },
                FixedValue::Text(s) => {
            ...
            RustType::Primitive(primitive) => match primitive {
                ...
                Primitive::Bytes => {
                    write_string_sz(body, "write_bytes", serializer_use, expr, line_ender, &encoding_var);
                },
                Primitive::Str => {
                    write_string_sz(body, "write_text", serializer_use, expr, line_ender, &encoding_var);
                },
                Primitive::I32 |
                Primitive::I64 => {
                    let mut pos = Block::new(&format!("if {} >= 0", expr_deref));
                    write_using_sz(&mut pos, "write_unsigned_integer", serializer_use, &format!("{} as u64", expr_deref), line_ender, &encoding_var);
                    body.push_block(pos);
                    let mut neg = Block::new("else");
                    write_using_sz(&mut neg, "write_negative_integer", serializer_use, &format!("{} as i128", expr_deref), line_ender, &encoding_var);
                    body.push_block(neg);
                },
                Primitive::U32 => {
                    write_using_sz(body, "write_unsigned_integer", serializer_use, &format!("{} as u64", expr_deref), line_ender, &encoding_var);
                },
                Primitive::U64 => {
                    write_using_sz(body, "write_unsigned_integer", serializer_use, &expr_deref, line_ender, &encoding_var);
                },
                Primitive::N64 => {
                    write_using_sz(body, "write_negative_integer", serializer_use, &expr_deref, line_ender, &encoding_var);
                },
            },
            RustType::Rust(t) => {
            ...
            RustType::Tagged(tag, ty) => {
                write_using_sz(body, "write_tag", serializer_use, &format!("{}u64", tag), "?;", &format!("{}{}_tag_encoding", enc_var_prefix, var_name));
                self.generate_serialize(types, ty, expr, expr_is_ref, var_name, body, is_end, serializer_name_overload, encoding_var_is_local);
            },
            RustType::Optional(ty) => {
                ...
                self.generate_serialize(types, ty, "x", true, var_name, &mut some_block, true, serializer_name_overload, encoding_var_is_local);
                some_block.after(",");
            ...
            RustType::Alias(_ident, ty) => self.generate_serialize(types, ty, expr, expr_is_ref, var_name, body, is_end, serializer_name_overload, encoding_var_is_local),
        };
    }
        ...
        match rust_type {
            ...
            RustType::Map(key_type, value_type) => {
                ...
                if !self.deserialize_generated_for_type(key_type) {
                    ...
                    // if let Some(tag) = tag {
                    //     deser_body.line(&format!("let tag = {}.tag()?;", deserializer_name));
...
fn create_serialize_impls(ident: &RustIdent, rep: Option<Representation>, tag: Option<usize>, definite_len: Option<String>, use_this_encoding: Option<&str>, generate_serialize_embedded: bool) -> (codegen::Function, codegen::Impl, Option<codegen::Impl>) {
    ...
    if let Some(tag) = tag {
        write_using_sz(&mut ser_func, "write_tag", "serializer", &format!("{}u64", tag), "?;", "self.tag_encoding");
    }
...
fn codegen_struct(gen_scope: &mut GenerationScope, types: &IntermediateTypes, name: &RustIdent, tag: Option<usize>, record: &RustRecord) {
    ...
    for field in &record.fields {
        ...
        // but even fixed values still need to keep track of their encodings if that is enabled
        if CLI_ARGS.preserve_encodings {
            for field_enc in encoding_fields(&field.name, &field.rust_type.clone().resolve_aliases()) {
                native_struct.field(&field_enc.field_name, field_enc.type_name);
                native_new_block.line(format!("{}: {},", field_enc.field_name, field_enc.default_expr));
            }
            if record.rep == Representation::Map {
                let key_enc = key_encoding_field(&field.name, field.key.as_ref().unwrap());
                native_struct.field(&key_enc.field_name, key_enc.type_name);
                native_new_block.line(format!("{}: {},", key_enc.field_name, key_enc.default_expr));
            }
        }
    }
    let encoding_var = if CLI_ARGS.preserve_encodings {
        native_struct.field("encoding", "LenEncoding");
        native_new_block.line("encoding: LenEncoding::default(),");
        if tag.is_some() {
            native_struct.field("tag_encoding", "Option<Sz>");
            native_new_block.line("tag_encoding: None,");
        }
        if record.rep == Representation::Map {
    ...
    let ctor_block = match record.rep {
        Representation::Array => {
            ...
            for field in record.fields.iter() {
                if field.optional {
                    ...
                    gen_scope.generate_serialize(types, &field.rust_type, "field", true, &field.name, &mut optional_array_ser_block, false, None, false);
                ...
                if CLI_ARGS.preserve_encodings {
                    for field_enc in encoding_fields(&field.name, &field.rust_type.clone().resolve_aliases()) {
                        deser_ret.line(format!("{},", field_enc.field_name));
                    }
                }
            }
            if let Some(var_name) = encoding_var {
                ...
                if types.is_plain_group(name) {
                    deser_ret.line(format!("{}: len.into(),", var_name));
                } else {
        ...
        Representation::Map => {
            ...
            for (field_index, field) in record.canonical_ordering() {
                ...
                let key_encoding_var = format!("self.{}", key_encoding_field(&field.name, &key).field_name);
                ...
                // serialize value
                gen_scope.generate_serialize(types, &field.rust_type, &data_name, expr_is_ref, &field.name, &mut map_ser_content, false, None, false);
            ...
            for field in &record.fields {
                ...
                if CLI_ARGS.preserve_encodings {
                    for field_enc in encoding_fields(&field.name, &field.rust_type.clone().resolve_aliases()) {
                        ctor_block.line(format!("{},", field_enc.field_name));
                    }
                    let key_enc = key_encoding_field(&field.name, field.key.as_ref().unwrap());
                    ctor_block.line(format!("{},", key_enc.field_name));
                }
            }
            if CLI_ARGS.preserve_encodings {
                ctor_block
    ...
    end_len(&mut ser_func, "serializer", "self.encoding", true);
    ...
    for variant in variants.iter() {
        ...
        if variant.serialize_as_embedded_group {
            ...
            //if variant.rust_type.is_serialize_multiline() {
                ...
                gen_scope.generate_serialize(types, &variant.rust_type, "x", true, &variant.name.to_string(), &mut case_block, !write_break, None, false);
                if write_break {
...
fn generate_wrapper_struct(gen_scope: &mut GenerationScope, types: &IntermediateTypes, type_name: &RustIdent, field_type: &RustType, control: Option<&ControlOperator>) {
    if control.is_some() {
    ...
    let field_or_ctrl = match control {
    ...
    let enc_fields = if CLI_ARGS.preserve_encodings {
        // println!("name: {}, type: {}", &field_enc.field_name, &field_enc.type_name);
        s.field("inner", field_or_ctrl.for_rust_member(false));
        let enc_fields = encoding_fields("inner", &field_or_ctrl.clone().resolve_aliases());
        for field_enc in &enc_fields {
            s.field(&field_enc.field_name, &field_enc.type_name);
        }
        Some(enc_fields)
    } else {
        s.tuple_field(field_or_ctrl.for_rust_member(false));
        None
    };
    if field_or_ctrl.is_copy() {
        s.derive("Copy");
    }
    ...
    if field_or_ctrl.is_copy() {
        get
            .ret(field_or_ctrl.for_rust_member(false))
            .line(field_or_ctrl.clone_if_not_copy(self_var));
    } else {
        get
            .ret(format!("&{}", field_or_ctrl.for_rust_member(false)))
    ...
    match control {
        Some(ControlOperator::Control(_)) => {
            let inner_se = "inner_se";
            ser_func.line(&format!("let mut {} = Serializer::new_vec();", inner_se));
            gen_scope.generate_serialize(types, field_or_ctrl, self_var, false, "inner", &mut ser_func, false, Some((inner_se, true)), false);
            ser_func.line(&format!("{}.write_bytes(&{}.finalize())", "serializer", inner_se));
        },
        _ => {
            gen_scope.generate_serialize(types, field_or_ctrl, self_var, false, "inner", &mut ser_func, true, None, false);
        },
    };
    ser_impl.push_fn(ser_func);
    let mut deser_func = make_deserialization_function("deserialize");
    ...
    deser_impl.impl_trait("Deserialize");
    if let RustType::Rust(id) = field_or_ctrl {
    ...
    let mut new_func = codegen::Function::new("new");
    new_func
        .arg("inner", field_or_ctrl.for_rust_move())
        .vis("pub");
    let var_names_str = if CLI_ARGS.preserve_encodings {
        encoding_var_names_str("inner", field_or_ctrl)
    } else {
        "inner".to_owned()
    };
    let from_impl = if let Some(&ControlOperator::Range((min, max))) = control {
        let (before, after) = if var_names_str.is_empty() {
            ("".to_owned(), "")
        } else {
            (format!("let {} = ", var_names_str), ";")
        };
        gen_scope.generate_deserialize(types, field_or_ctrl, "inner", &before, after, false, false, vec![], &mut deser_func, None);
        
        let against = match field_or_ctrl.strip_tags_and_aliases() {
            RustType::Primitive(p) => match p {
                Primitive::Bytes |
        ...
        let mut check = match (min, max) {
            (Some(min), Some(max)) => if min == max {
                ...
                let non_negative = match field_or_ctrl.strip_tags_and_aliases() {
                    RustType::Primitive(p) => match p {
                        Primitive::Bytes |
                        Primitive::Str => true,
                        Primitive::Bool |
        ...
        try_from
            ...
            .impl_trait(format!("TryFrom<{}>", field_or_ctrl.for_rust_member(false)))
            .new_fn("try_from")
            .arg("inner", field_or_ctrl.for_rust_member(false))
            .ret("Result<Self, Self::Error>")
            .line(format!("{}::new({})", type_name, ToWasmBoundaryOperations::format(field_or_ctrl.from_wasm_boundary_clone("inner", false).into_iter())));
        try_from
    } else {
        // let field_type_tagged = if let Some(t) = tag {
        //     RustType::Tagged(t, Box::new(field_or_ctrl.clone()))
        // } else {
        //     field_or_ctrl.clone()
        // };
        ...
        let name_overload = match control {
            Some(ControlOperator::Control(_)) => {
                let name_overload = "inner_de";
                deser_func.line(&format!("let mut {} = &mut Deserializer::from(std::io::Cursor::new(raw.bytes()?));", name_overload));
                Some(name_overload)
            },
            _ => None,
        };
        ...
        if let Some(enc_fields) = &enc_fields {
            ...
            gen_scope.generate_deserialize(types, &field_or_ctrl, "inner", &before, after, false, false, vec![], &mut deser_func, name_overload);
            ...
            gen_scope.generate_deserialize(types, &field_or_ctrl, "inner", "Ok(Self(", "))", false, false, vec![], &mut deser_func, name_overload);
            new_func.line("Self(inner)");
        }
	    
        let mut from = codegen::Impl::new(&type_name.to_string());
        from
            .impl_trait(format!("From<{}>", field_or_ctrl.for_rust_member(false)))
            .new_fn("from")
            .arg("inner", field_or_ctrl.for_rust_member(false))
            .ret("Self")
            .line(format!("{}::new({})", type_name, ToWasmBoundaryOperations::format(field_or_ctrl.from_wasm_boundary_clone("inner", false).into_iter())));
    ...
    let mut from_inner_impl = codegen::Impl::new(field_or_ctrl.for_rust_member(false));
    from_inner_impl
