...
"#,
    ...
    assert_eq!(
        common_import_flavor, None,
        "config has no hand-flavor input key"
    );
    assert!(
        config_runtime_decision_owned,
        "config expansion must mark its closed runtime decision internally"
    );
    assert_eq!(
        wasm_cbor_json_api_macro,
...
/// - `common-import-flavor` is deliberately hand-flag-only: config's existing `[runtime]` carrier
///   decision already owns the shared runtime contract, and adding a second config input would
///   re-adjudicate that maintainer-closed decision from a committed record. `config-runtime-
///   decision-owned` is the corresponding internal `#[clap(skip)]` marker, not a public knob.
#[test]
fn config_keys_match_cli_fields() {
    ...
    let mut cli_keys = struct_field_keys(concat!(env!("CARGO_MANIFEST_DIR"), "/src/cli.rs"), "Cli");
    let internal_only: BTreeSet<String> = ["common-import-flavor", "config-runtime-decision-owned"]
        .into_iter()
        .map(str::to_owned)
        .collect();
    for key in &internal_only {
        assert!(
            cli_keys.remove(key),
            "config's explicit internal-only CLI exception `{key}` no longer names a Cli field"
        );
    }
...
///    provably complete: it compares every externally selected field, so a derived flag the hand
///    list forgot fails here even when its effect is unreachable in this fixture. The sole exception
///    is the internal config-origin marker, which deliberately differs by construction and is
///    asserted separately. Without this, an acceptance test that compared only bytes would silently
///    stop covering any derivation whose output effect this particular spec does not reach.
...
fn a_whole_config_generates_what_the_hand_written_flags_generate() {
    ...
    // (1) Completeness: every externally selected field of every crate's `Cli`, with each run's own
    // root erased so the only thing that can differ is a flag. The internal config-origin marker is
    // asserted at its expected origin and then normalized for this comparison. Reported line by
    // line rather than through `assert_eq!`, because a `Cli`'s pretty-printed debug is forty fields
    // and the escaped one-line rendering of two of them buries the single field that moved.
    for ((name, config_cli), (_, argv)) in expanded.iter().zip(hand.iter()) {
        assert!(
            config_cli.config_runtime_decision_owned,
            "`{name}` expanded from config must carry the internal config-runtime ownership marker"
        );
        let hand_cli = Cli::parse_from(argv);
        assert!(
            !hand_cli.config_runtime_decision_owned,
            "`{name}` parsed from hand-written flags must not carry the internal config-runtime ownership marker"
        );
        let erase = |cli: &Cli, at: &Path| {
            format!("{cli:#?}")
                .replace(&at.to_string_lossy().into_owned(), "<ROOT>")
                .replace(
                    "config_runtime_decision_owned: true",
                    "config_runtime_decision_owned: <CONFIG_RUNTIME_DECISION>",
                )
                .replace(
                    "config_runtime_decision_owned: false",
                    "config_runtime_decision_owned: <CONFIG_RUNTIME_DECISION>",
                )
        };
