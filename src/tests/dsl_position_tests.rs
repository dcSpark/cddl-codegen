...
const GRID: &[Cell] = &[
    ...
    // 23-iii. REJECT: the member slot of a SINGLE-ENTRY group-choice arm — a field/member position
    //      that mints no record, so the pair (COMPLETE, unlike 23-i/23-ii) has no field to ride and
    //      the arm's codec is generated by the enum. Cell 23 is the control that keeps this
    //      attributable to the one-entry ARM rather than to the pair's completeness. The remedy is
    //      the member's own type rule, which routes into this very arm.
    Cell {
        directive: "@custom_serialize+deserialize",
        position: "single-entry-group-choice-arm",
        spec: "holder = [a: uint\n// f: bytes ; @custom_serialize my_ser @custom_deserialize my_deser\n]\n",
        flags: &[],
        wasm: false,
        expect: Expect::Reject(
            "the custom (de)serializer pair cannot be honored on a single-entry arm",
        ),
    },
