...
/// Surfaces the own-spec extern re-export contract three ways so a stale hand-written thin root can't
/// leave a user staring at a bare E0432 (the generated glue's `pub use crate::<Name>;` resolves only
/// if the crate-root `lib.rs` re-exports `<Name>` from wherever the user defined it):
///   (1) run-output print — the required per-crate re-export list (rust and wasm) on stdout;
///   (2) contract comment — a `//` group above each generated scope's glue restating the contract;
///   (3) generation-time diagnostic — a stderr warning when a seed-once `lib.rs` ALREADY EXISTS and
...
/// `cargo`-guarded (the print/warning are only observable through the subprocess).
#[test]
fn extern_reexport_contract_surfaced() {
    if !tool_exists("cargo") {
        return;
    }

    // Every `.rs` under either crate's `generated/**`, rel-path -> bytes, for the byte-identity check.
    fn generated_snapshot(out: &std::path::Path) -> std::collections::BTreeMap<String, String> {
        let mut map = std::collections::BTreeMap::new();
        for sub in ["rust/src/generated", "wasm/src/generated"] {
            let base = out.join(sub);
            if !base.exists() {
                continue;
            }
    ...
    let root = std::env::temp_dir().join(format!(
        "cddl_codegen_extern_contract_{:016x}",
        checkout_hash()
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();

    // --- Main flow: a plain in-crate extern used by a generated wrapper (rust + wasm glue). ---
    ...
    // (1) Fresh seed: the print names the required re-exports for BOTH crates; NO diagnostic fires
    // (the seed-once roots did not pre-exist, so the diagnostic branch is skipped entirely).
    let (stdout1, stderr1) = run_gen(&input, &out);
    assert!(
        stdout1.contains("the hand-written rust/src/lib.rs must re-export")
            && stdout1.contains("pub use <your_module>::MyExt;"),
        "run-output print must list the rust required re-exports:\n{stdout1}"
    );
    assert!(
        stdout1.contains("the hand-written wasm/src/lib.rs must re-export"),
        "run-output print must list the wasm required re-exports:\n{stdout1}"
    );
    assert!(
        !stderr1.contains("is missing crate-root re-exports"),
        "a fresh seed must NOT emit the missing-re-export diagnostic:\n{stderr1}"
    );

    // (2) The contract comment sits above the glue in each generated scope mod.rs.
    for crate_mod in ["rust/src/generated/mod.rs", "wasm/src/generated/mod.rs"] {
        let text = std::fs::read_to_string(out.join(crate_mod)).unwrap();
        assert!(
            text.contains("cddl-codegen extern re-export contract:")
                && text.contains("pub use crate::MyExt;"),
            "{crate_mod} must carry the contract comment above the glue:\n{text}"
        );
    }

    let generated_after_seed = generated_snapshot(&out);

    // (3a) Second run over the still-stale seeded roots (they carry `pub use generated::*;` — the
    // tool's OWN glob, which must NOT suppress — but not `MyExt`): the diagnostic fires for BOTH
    // crates, naming the missing ident and the fix.
    let (_stdout2, stderr2) = run_gen(&input, &out);
    assert!(
        stderr2.contains("rust/src/lib.rs is missing crate-root re-exports")
            && stderr2.contains("MyExt")
            && stderr2.contains("pub use <your_module>::<Name>;"),
        "a stale rust seed missing a required re-export must warn with the fix:\n{stderr2}"
    );
    assert!(
        stderr2.contains("wasm/src/lib.rs is missing crate-root re-exports"),
        "a stale wasm seed missing a required re-export must warn:\n{stderr2}"
    );

    // The diagnostic reads prior output but writes ZERO bytes: generated/** is byte-identical.
    assert_eq!(
        generated_after_seed,
        generated_snapshot(&out),
        "the missing-re-export diagnostic must not change any generated byte (run-twice=run-once)"
    );

    // (3b) Satisfy the contract on both roots: the diagnostic goes silent.
    ...
    let (_stdout3, stderr3) = run_gen(&input, &out);
    assert!(
        !stderr3.contains("is missing crate-root re-exports"),
        "a satisfied root must not warn:\n{stderr3}"
    );

    // (3c) A foreign glob re-exporter (`pub use utils::*;`, path last-segment != `generated`) the
    // name scan can't see through: suppress the per-name warning rather than false-positive.
    std::fs::write(
        out.join("rust/src/lib.rs"),
        "mod generated;\npub use generated::*;\npub mod utils;\npub use utils::*;\n",
    )
    .unwrap();
    let (_stdout4, stderr4) = run_gen(&input, &out);
    assert!(
        !stderr4.contains("rust/src/lib.rs is missing crate-root re-exports"),
        "a non-generated glob re-exporter must suppress the rust per-name warning:\n{stderr4}"
    );

    // --- Raw-bytes-flavor coverage: the `<Base>RawBytes` flavor name surfaces in the print. ---
    let rb_input = root.join("rawbytes.cddl");
    std::fs::write(
        &rb_input,
    ...
    .unwrap();
    let rb_out = root.join("rb_crate");
    let (rb_stdout, _rb_stderr) = run_gen(&rb_input, &rb_out);
    assert!(
        rb_stdout.contains("pub use <your_module>::ExtSetRawBytes;"),
        "the @raw_bytes_flavor <Base>RawBytes flavor name must surface in the print:\n{rb_stdout}"
    );
...
fn open_struct_map_ignore_e2e() {
    ...
    // fixed keys win over the rest row (the key-dispatch deviation), duplicate unknown keys are consumed silently while a
    // duplicate FIXED key still errors, and a no-unknown map is byte-identical to the closed struct.
    // `--json-serde-derives=true` also pins the JSON posture: an ignore struct is a CLOSED serde struct (declared
...
/// `--json-schema-export` WITHOUT `--json-serde-derives`. Nothing couples the two flags — neither
/// `cli.rs`'s validation nor `api.rs` rejects or implies the pairing — and every other
/// `--json-schema-export` cell in this file passes both, so before this one the uncoupled
/// combination was generated by nobody and compiled by nobody.
///
/// What it pins, and why each half matters:
///
/// 1. **`rust/Cargo.toml` declares `serde_json`.** Its condition is `--json-serde-derives` OR
///    `--json-schema-export`, not the first alone: under the schema flag the rust crate hosts
///    `json_schema_gen.rs`, whose reference-closure check walks a `serde_json::Value`, while no type
///    in the crate derives `serde::Serialize`. Narrowing the condition back to `--json-serde-derives`
///    makes the rust crate an `E0433` naming `serde_json`, and this cell is what sees it.
/// 2. **`rust/src/generated/json_schema_gen.rs` is written** — the in-crate home of the helpers (no
///    `--common-import-override` here), so the `use cddl_lib::json_schema_gen::…;` the json-gen crate
///    emits has something to resolve to.
/// 3. **The json-gen crate actually builds and runs**, writing a document with a `$defs` entry per
///    row. A `cargo run` rather than a `cargo check`: the reference-closure check runs at runtime,
///    and a document that never reaches disk would satisfy a check-only verdict.
///
/// Scope, stated honestly: this exercises the IN-CRATE layout only. The `--export-static-crate` side
/// of the same widened condition is covered by generation-level manifest assertions in
/// `export_static_crate_writes_composed_runtime_and_manifest` leg (f), which compiles nothing.
///
/// Its own self-contained harness rather than a `run_test` row for the same reason
/// `json_schema_dep_threading` has one (and modelled on it, including the export-dir reset of the
/// merged manifests and the seed-once crate roots): `run_test` would additionally build and test the
/// rust and wasm crates, and the subject here is the json-gen crate alone.
#[test]
fn json_schema_export_without_serde_derives() {
    use std::str::FromStr;
    let test_path = std::path::PathBuf::from_str("tests")
        .unwrap()
        .join("json-schema-no-serde");
    let export_path = test_path.join("export");
    println!("--------- running json-schema-export-without-serde-derives test ---------");
    for manifest in [
        "rust/Cargo.toml",
        "wasm/Cargo.toml",
        "wasm/json-gen/Cargo.toml",
    ...
    // `--json-serde-derives` is deliberately left off the command line entirely (its default is
    // false) rather than passed as `=false`, so the cell reads as the command line of a user who
    // wants schemas and nothing else.
    let generate = codegen_cmd()
        .arg(format!("--output={}", export_path.to_str().unwrap()))
        .arg(format!(
            "--input={}",
    ...
    assert!(
        generate.status.success(),
        "generation must succeed with --json-schema-export and no --json-serde-derives"
    );

    let rust_manifest = std::fs::read_to_string(export_path.join("rust/Cargo.toml")).unwrap();
    assert!(
        rust_manifest.contains("serde_json = "),
        "rust/Cargo.toml must declare serde_json under --json-schema-export alone — \
         json_schema_gen.rs walks a serde_json::Value:\n{rust_manifest}"
    );
    assert!(
        rust_manifest.contains("schemars = "),
        "rust/Cargo.toml must declare schemars under --json-schema-export:\n{rust_manifest}"
    );
    // The negative twin: `serde` itself is asserted by `--json-serde-derives` ONLY, and this crate
    // derives no `Serialize`/`Deserialize`. Pinning its absence is what keeps the widening above
    // from being read as "assert everything json-shaped under either flag".
    assert!(
        !rust_manifest.contains("\nserde = "),
        "rust/Cargo.toml must NOT declare serde — no type derives it without \
         --json-serde-derives:\n{rust_manifest}"
    );
    assert!(
        export_path
            .join("rust/src/generated/json_schema_gen.rs")
            .exists(),
        "the in-crate helper module must be written under --json-schema-export"
    );
    ...
    assert!(
        run.status.success(),
        "the json-gen run must SUCCEED with --json-serde-derives off — the crate did not compile \
         or the document did not resolve:\n{stderr}"
    );
...
///   (f) the same leg with `--json-serde-derives` dropped — the uncoupled combination (nothing in
///       `cli.rs` or `api.rs` implies one flag from the other) — still asserts `serde_json`, because
///       the exported `json_schema_gen.rs` walks a `serde_json::Value` whatever the derives flag
///       says, and does NOT assert `serde`, which only `--json-serde-derives` needs.
...
fn export_static_crate_writes_composed_runtime_and_manifest() {
    ...
    // nothing in `cli.rs` or `api.rs` implies or rejects either from the other — so this is a legal
    // command line, and it is the one that decides `serde_json`'s condition in the static-runtime
    // changeset: `any_cbor_json.rs` needs it under the derives flag, `json_schema_gen.rs` needs it
    // under the schema flag, and only the OR of the two covers both. Generation-level only (the
    // exported crate has no root of its own for the tool to compile); the compiling twin for the
    // in-crate layout is `json_schema_export_without_serde_derives`.
    let no_serde_crate = scratch.join("no-serde-runtime");
    let cli_no_serde = crate::cli::Cli::parse_from([
        "cddl-codegen",
        "--input",
        input.to_str().unwrap(),
        "--output",
        scratch.join("no-serde-crate").to_str().unwrap(),
        "--wasm=false",
        "--json-schema-export=true",
        "--common-import-override=cml_core",
        "--export-static-crate",
        no_serde_crate.to_str().unwrap(),
    ]);
    crate::api::generate_to_disk(&cli_no_serde).unwrap();
    assert!(
        no_serde_crate
            .join("src")
            .join("json_schema_gen.rs")
            .exists(),
        "json_schema_gen.rs must be exported under --json-schema-export alone"
    );
    let no_serde_manifest = std::fs::read_to_string(no_serde_crate.join("Cargo.toml")).unwrap();
    for dep in ["schemars", "serde_json"] {
        assert!(
            no_serde_manifest.contains(&format!("{dep} = ")),
            "the exported manifest must declare {dep} under --json-schema-export ALONE — \
             json_schema_gen.rs references it whatever --json-serde-derives says:\n{no_serde_manifest}"
        );
    }
    assert!(
        !no_serde_manifest.contains("\nserde = "),
