...
use crate::comment_ast::{metadata_from_comments, RuleMetadata};
        ...
        Type2::Typename {
            ...
            match control {
                Some(control) => {
                    ...
                    match control {
                        ControlOperator::Range(min_max) => match cddl_ident.to_string().as_str() {
                            ...
                            match control {
                                Some(ControlOperator::CBOR(_ty)) => {
                                    ...
                                    let base_type = types
                                        ...
                                        .expect(&format!(
                                            "Please move definition for {} above {}",
                                            type_name, ident
                                        ));
                                    types.register_type_alias(
                                        type_name.clone(),
                                        base_type.as_bytes().tag(tag_unwrap),
                                        true,
                                        true,
                                    );
                                }
...
pub fn create_variants_from_type_choices(
    ...
    type_choices
        ...
        .map(|choice| {
            ...
            let variant_name = append_number_if_duplicate(
                &mut variant_names_used,
                rust_type.for_variant().to_string(),
            );
    ...
    match control {
        ...
        Some(ControlOperator::Range(min_max)) => match &type1.type2 {
            Type2::Typename { ident, .. }
                if ident.to_string() == "uint" || ident.to_string() == "int" =>
            {
                match range_to_primitive(min_max.0, min_max.1) {
                    Some(t) => t.into(),
                    None => panic!(
