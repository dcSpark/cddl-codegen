    ...
    if include_any_cbor {
        ...
        if cli.json_schema_export {
            ...
            if include_ordered_set {
                any_cbor_rs.push_str(&std::fs::read_to_string(
                    cli.static_dir
                        .join("any_cbor_bounded_ordered_set_schemars.rs"),
                )?);
            }
        }
        out.push((
...
/// order. `note!`: stdout at the DEFAULT level, because it is user-facing run output rather than a
/// diagnostic — the generated glue does not resolve until the user acts on it, so it must not need a
/// flag to be seen, and a caller pipes stdout.
fn print_required_reexports(root_rel_path: &str, required: &std::collections::BTreeSet<String>) {
    ...
    for name in required {
        crate::note!("    pub use <your_module>::{name};");
    }
}
...
pub(crate) fn rustfmt_source_with<'a>(
    ...
    match String::from_utf8(output) {
        Ok(bindings) => match status.code() {
            ...
            Some(3) => {
                // A diagnostic about the output, on stderr with the rustfmt errors it accompanies.
                crate::warn!("Rustfmt could not format some lines.");
...
impl GenerationScope {
    ...
    pub fn export(
        ...
        // a function of the IR and the flag set) and merged against whatever is already on disk by
        // the write tail, which owns every prior-output read.
        ...
        // them and recomputes them from each file's own final composed content, so the tail writes
        // exactly what is composed here. The `static/*.rs` sources therefore hold no hand-written
        // alloc imports of their
        ...
        // `--export-static-crate`: ADDITIONALLY compose the rust runtime for the named crate's
        // `src/`, independent of the in-crate export above (the upgrade path for
        ...
        // appended).
        let static_crate = match &cli.export_static_crate {
            None => None,
            Some(export_crate) => {
        ...
        // Everything from here is disk: the manifest merges, the comment-preservation overlay, the
        // write loop and the two scans. It runs in `write_tail` — the one module that reads prior
        // output — and takes plain values, so it is drivable from a test without an IR or a
        // `GenerationScope` (see that module's docs and `src/tests/write_tail_tests.rs`).
        crate::generation::write_tail::WriteTailPlan {
            files,
            rust_dir,
            output_dir: cli.output.clone(),
            manifest_ops,
            prune_config: self.build_prune_config(cli),
            preserve_comments: cli.preserve_comments,
            component: cli.component,
            composed_runtime_files,
            required_rust_reexports: self.required_rust_reexports.clone(),
